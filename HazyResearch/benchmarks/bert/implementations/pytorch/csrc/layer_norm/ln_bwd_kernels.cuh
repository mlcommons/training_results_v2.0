...
void ln_bwd_kernel(layer_norm::BwdParams params) {
    ...
    for( int row = r; row < params.rows; row += params.ctas_per_col * ROWS_PER_CTA ) {
        ...
        for( int it = 0; it < LDGS; it++ ) {
            ...
            dmask[it].load_from(params.dmask, idx);
            idx += Ktraits::VEC_COLS_PER_LDG;
        }
        ...
        for( int it = 0; it < LDGS; it++ ) {
            ...
            for( int jt = 0; jt < NUM_ELTS; jt++ ) {
                ...
                dx1[it].data.elt[jt] = dx_tmp;
                dx0[it].data.elt[jt] = dmask[it].data.elt[jt] ? dx_tmp * params.dropout_scale : 0.f;
            }
            dx1[it].store_to(params.dx1, idx);
