...
from model.ops.k_partitioning import k_partition
...
def parse_arguments():
    ...
    parser.add_argument("--unpad",
                        ...
                        action='store_true',
                        help="Whether to run with unpadding.")
    parser.add_argument("--unpad_stream_attn",
                        default=False,
                        action='store_true',
                        help="Whether to run streaming attention with unpadding.")
    ...
    args.name = os.environ.get('NAME', None)
    args.group = os.environ.get('GROUP', None)
    args.wandb_enabled = args.name is not None or args.group is not None
...
def remap_fused_dense_gelu_dense_model_parameters(model_dict, config):
    ...
    # TD [2022-05-09]: this doesn't seem to affect anything, I'm disabling it for now.
    # if getattr(config, 'vocab_size_og', None) is not None and config.vocab_size_og < config.vocab_size:
    #     res_dict['cls.predictions.bias'][config.vocab_size_og:] = -100.0
    model_dict.clear()
    return res_dict
...
def prepare_model_and_optimizer(args, device, stream):
    ...
    config.unpad_stream_attn = args.unpad_stream_attn
...
def exchange_padding_fast(device, input_ids, segment_ids, input_mask, masked_lm_labels, next_sentence_labels, max_batch_size):
    ...
    partition_indices = k_partition(seqlens_, ngpus) == igpu
    seqlens_, indices_ = seqlens_[partition_indices], indices[partition_indices]
    # if igpu == 0:
    #     print(f'Fraction below 128: {(seqlens_ <= 128).float().mean()}')
    #     print(f'Fraction from 128 to 256: {(torch.logical_and(seqlens_ > 128, seqlens_ <= 256)).float().mean()}')
    #     print(f'Fraction above 256: {(seqlens_ > 256).float().mean()}')
    # We might need to remove the empty sequences we added to the batch
    if pad_size > 0:
        indices_ = indices_[seqlens_ > 0]
    input_ids_sorted = input_ids_.view(nseqs * ngpus, ntokensperseq)[indices_, :].contiguous()
    segment_ids_sorted = segment_ids_.view(nseqs* ngpus, ntokensperseq)[indices_, :].contiguous()
    input_mask_sorted = input_mask_.view(nseqs* ngpus, ntokensperseq)[indices_, :].contiguous()
    masked_lm_labels_sorted = masked_lm_labels_.view(nseqs* ngpus, ntokensperseq)[indices_, :].contiguous()
...
def main():
    ...
    logdir = 'None' if args.group is None else args.group
    # mlperf_logger.mlperf_submission_log('bert', logdir, args.seed)
    ...
    if args.wandb_enabled and utils.is_main_process():
        wandb.init(name=args.name, group=args.group, project='attention', config=args)
                ...
                for step, batch in enumerate(train_dataloader):
                            ...
                            if skip_interval > 0:
                                ...
                                if utils.is_main_process():
                                    ...
                                    if args.wandb_enabled:
                                        wandb.log({'val/loss': eval_avg_loss, 'val/acc': eval_avg_mlm_accuracy, 'trainer/global_step': global_step})
                                    print({"global_step": global_step, "eval_loss": eval_avg_loss, "eval_mlm_accuracy":eval_avg_mlm_accuracy})
                    ...
                    if args.log_freq > 0 and training_steps % (args.log_freq * args.gradient_accumulation_steps) == 0:
                        ...
                        if utils.is_main_process():
                            ...
                            if args.wandb_enabled:
                                wandb.log({
                                    "trainer/global_step": now_step,
                                    "train/step_loss": loss.item() * args.gradient_accumulation_steps / divisor,
                                    'train/avg_loss': average_loss / (args.log_freq * divisor),
                                    'lr': now_lr,
                                    'train/seq_per_s': training_perf,
                                })
                            # for DLFW CI/CD
...
if __name__ == "__main__":
    ...
    if utils.is_main_process():
        ...
        training_perf = global_batch_size(args) \
                        * (global_step - args.resume_step + skipped_steps.item()) / train_time_raw
        if args.do_train:
            print({"e2e_time": e2e_time, "training_sequences_per_second": training_perf,
                                             "final_loss": final_loss, "raw_train_time": train_time_raw })
            if args.wandb_enabled:
                wandb.log({'e2e_time': e2e_time, 'training_seq_per_s': training_perf,
