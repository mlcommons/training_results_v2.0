...
from model.ops.bert_padding import unpad_input, pad_input, index_first_axis
from model.ops.stream_attn_interface import stream_attn_func
from model.losses.cross_entropy_apex import CrossEntropyLossApex
from model.ops.fused_dense import fused_dense_function_td
...
class BertStreamSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, cu_seqlens, max_seqlen_in_batch):
        ...
        context = stream_attn_func(qkv, cu_seqlens, self.p_dropout if self.training else 0.0,
                                   max_seqlen_in_batch)
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        if not self.fused_dropout_add_ln:
            ...
            hidden_states = dropout_add_layer_norm(hidden_states, input_tensor,
                                                   ...
                                                   # self.LayerNorm.eps)
                                                   self.LayerNorm.epsilon)
        return hidden_states


class BertStreamAttention(nn.Module):

    def __init__(self, config):
        super().__init__()
        self.self = BertStreamSelfAttention(config)
...
class BertFusedMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.fused_dropout_add_ln = getattr(config, 'fused_dropout_add_ln', False)
        # self.dense_gelu_dense = FusedDenseGeluDenseTD(config.hidden_size, config.intermediate_size,
        #                                               config.hidden_size)
        self.dense_gelu_dense = FusedDenseResGeluDense(config.hidden_size, config.intermediate_size,
                                                       config.hidden_size)
        # self.fc1 = FusedDenseTD(config.hidden_size, config.intermediate_size)
        # self.fc1 = FusedDenseResidual(config.hidden_size, config.intermediate_size)
        # self.fc2 = FusedDenseTD(config.intermediate_size, config.hidden_size)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_tensor):
        # hidden_states = self.dense_gelu_dense(input_tensor)
        hidden_states, input_tensor = self.dense_gelu_dense(input_tensor)
        # hidden_states = self.fc2(F.gelu(self.fc1(input_tensor), approximate='tanh'))
        # fc1_output, input_tensor = self.fc1(input_tensor)
        # hidden_states = self.fc2(F.gelu(fc1_output, approximate='tanh'))
        if not self.fused_dropout_add_ln:
            hidden_states = self.LayerNorm(self.dropout(hidden_states) + input_tensor)
        else:
            hidden_states = dropout_add_layer_norm(hidden_states, input_tensor,
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.stream_attn = config.unpad
        if config.unpad_stream_attn:
            self.attention = BertStreamAttention(config)
        else:
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.unpad_stream_attn = config.unpad_stream_attn

    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True,
                subset_mask=None):
        all_encoder_layers = []
        if self.unpad_stream_attn:
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.last_layer_subset:
            assert getattr(config, 'unpad_stream_attn', False)
