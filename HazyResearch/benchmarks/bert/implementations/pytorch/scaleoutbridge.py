...
class ScaleoutBridge(object):
    ...
    def print_tag(self, tag, dur, deviceid):
        mlperf_logger.log_event(key=tag, value={'r':deviceid, 't':dur}, log_all_ranks=True, sync=False)
    ...
    def empty_q(self, q, tag):
        while not q.empty():
            atuple = q.get()
            mlperf_logger.log_event(key=tag, value={'r':atuple[1], 't':atuple[0]}, log_all_ranks=True, sync=False)
