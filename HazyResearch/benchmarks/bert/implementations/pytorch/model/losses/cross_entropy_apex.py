...
class SoftmaxCrossEntropyLossFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_loss):
        ...
        if not ctx.inplace_backward:
            grad_logits = xentropy_cuda.backward(grad_loss, logits, max_log_sum_exp, labels,
                                                 ctx.smoothing)
        else:
            grad_logits = xentropy_cuda_inplace.inplace_backward(grad_loss, logits, max_log_sum_exp,
                                                                 labels, ctx.smoothing)
...
class CrossEntropyLossApex(nn.Module):
    ...
    def forward(self, input, target):
        ...
        loss = SoftmaxCrossEntropyLossFn.apply(input, target, self.label_smoothing,
                                               self.ignore_index, is_half, self.inplace_backward)
