...
def make_data_loader(cfg, is_train=True, is_distributed=False, start_iter=0, random_number_generator=None, seed=41, shapes=None, hybrid_dataloader=None):
    dedicated_evaluation_ranks = max(0,cfg.DEDICATED_EVALUATION_RANKS)
    rank = get_rank()
    num_ranks = get_world_size()
    num_training_ranks = num_ranks - dedicated_evaluation_ranks
    num_evaluation_ranks = num_ranks if dedicated_evaluation_ranks == 0 else dedicated_evaluation_ranks
    if is_train:
        images_per_batch = cfg.SOLVER.IMS_PER_BATCH
        assert (
            images_per_batch % num_training_ranks == 0
        ), "SOLVER.IMS_PER_BATCH ({}) must be divisible by the number "
        "of GPUs ({}) used.".format(images_per_batch, num_training_ranks)
        images_per_gpu = images_per_batch // num_training_ranks
        shuffle = True
        num_iters = cfg.SOLVER.MAX_ITER
    else:
        images_per_batch = cfg.TEST.IMS_PER_BATCH
        assert (
            images_per_batch % num_evaluation_ranks == 0
        ), "TEST.IMS_PER_BATCH ({}) must be divisible by the number "
        "of GPUs ({}) used.".format(images_per_batch, num_evaluation_ranks)
        images_per_gpu = images_per_batch // num_evaluation_ranks
    ...
    if dedicated_evaluation_ranks > 0:
        if is_train:
            assert(rank < num_training_ranks), "Evaluation rank tried to create training dataset"
            num_ranks = num_training_ranks
        else:
            rank = rank - num_training_ranks
            assert(rank >= 0), "Training rank tried to create evaluation dataset"
    ...
    for dataset in datasets:
        ...
        if is_hybrid_loader:
            data_loader = HybridDataLoader3(cfg, images_per_gpu, cfg.DATALOADER.SIZE_DIVISIBILITY, shapes) if hybrid_dataloader is None else hybrid_dataloader
