...
class ScaleoutBridge(object):
    ...
    def print_tag(self, tag, dur):
        mllogger.event(log_rank=self.deviceid, key=tag, value={'r':self.deviceid, 't':dur})
    ...
    def printstat(self, tag):
        ...
        mllogger.event(log_rank=self.deviceid, key=tag+"_total", value={'r':self.deviceid, 't':total})
        mllogger.event(log_rank=self.deviceid, key=tag+"_count", value={'r':self.deviceid, 't':count})
        mllogger.event(log_rank=self.deviceid, key=tag+"_min", value={'r':self.deviceid, 't':minimum})
        mllogger.event(log_rank=self.deviceid, key=tag+"_max", value={'r':self.deviceid, 't':maximum})
...
class ScaleoutBridge_Epoch(object):
    ...
    def stop_epoch_prof(self):
        torch.cuda.synchronize()
        mllogger.event(key='epoch_time', value={'r':self.deviceid, 't':time()-self.start_time})
    def start_eval_prof(self):
        pass
    def stop_eval_prof(self):
        pass
