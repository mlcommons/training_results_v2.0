...
if [[ -n "${SLURM_LOCALID-}" ]] && [[ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]]; then
    ...
    if [[ "${DGXSYSTEM}" == 2R5500G5-Intelx8A100-SXM-80GB ]]; then
        CMD=( './bind.sh' '--cpu=2R5500G5-Intelx8A100-SXM-80GB_topology.sh' '--mem=2R5500G5-Intelx8A100-SXM-80GB_topology.sh' '--' ${NSYSCMD} 'python' '-u' )
    else
        CMD=( './bind.sh' '--cpu=exclusive' '--' ${NSYSCMD} 'python' '-u' )
    fi
else
    # docker or single gpu, no need to bind
	if [[ "${DGXSYSTEM}" == R5300G5x4A100-SXM-80GB ]]; then
	    CMD=('./bind.sh' '--cpu=R5300G5x4A100-SXM-80GB_bind.sh' '--mem=R5300G5x4A100-SXM-80GB_bind.sh'  '--' ${NSYSCMD} 'python' '-u' )
	else
	    CMD=( './bind.sh' '--cpu=exclusive' '--' ${NSYSCMD} 'python' '-u' )
	fi
fi
