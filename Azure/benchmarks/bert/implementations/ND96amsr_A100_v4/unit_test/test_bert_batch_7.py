...
class TestBert(unittest.TestCase):
  ...
  def test_bert_pooler_isolated(self):
    model_updated_input_mask = (1.0 - self.tf_input_mask) * -10000.0
    pyt_result = self.bert.bert.pooler(torch.from_numpy(gv.tf_tensors['bert/encoder/layer_23/output/layer_output']).to(self.device), model_updated_input_mask)
    pyt_result = pyt_result.detach().cpu().numpy()
  ...
  def test_bert_mlm_prediction_scores(self):
    ...
    if not self.bert_config.dense_seq_output:
      ...
      pyt_result = torch.index_select(pyt_result, 0, masked_lm_labels.view(-1).nonzero(as_tuple=True)[0])
    ...
    tf_result = torch.index_select(tf_result, 0, masked_lm_positions.view(-1).nonzero(as_tuple=True)[0].detach().cpu())
