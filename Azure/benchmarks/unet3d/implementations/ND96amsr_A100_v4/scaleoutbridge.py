...
class ScaleoutBridge(object):
    ...
    def print_tag(self, tag, dur):
        log_event(key=tag, value={'r':self.deviceid, 't':dur}, force=True)
    ...
    def printstat(self, tag):
        ...
        log_event(key=tag+'_total', value={'r':self.deviceid, 't':total}, force=True)
        log_event(key=tag+'_count', value={'r':self.deviceid, 't':count}, force=True)
        log_event(key=tag+'_min', value={'r':self.deviceid, 't':minimum}, force=True)
        log_event(key=tag+'_max', value={'r':self.deviceid, 't':maximum}, force=True)
...
class ScaleoutBridge_Epoch(object):
    ...
    def stop_epoch_prof(self):
        mx.nd.waitall()
        log_event(key='epoch_time', value={'r':self.deviceid, 't':time()-self.start_time}, force=True)
    def start_eval_prof(self):
        pass
    def stop_eval_prof(self):
        pass
