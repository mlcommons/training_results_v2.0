...
class AnchorGenerator(nn.Module):
    ...
    def forward_single_image(self, image_sizes, feature_map, cell_anchors, stride):
        ...
        grid_height, grid_width = feature_map.shape[-3:-1] if self.nhwc else feature_map.shape[-2:]
    ...
    def forward(self, image_sizes_tensor, feature_maps):
        grid_sizes = [feature_map.shape[-3:-1] if self.nhwc else feature_map.shape[-2:] for feature_map in feature_maps]
        per_image_boxlist = []
        anchors = []
...
def make_anchor_generator(config):
    ...
    anchor_generator = AnchorGenerator(
        anchor_sizes, aspect_ratios, anchor_stride, straddle_thresh, nhwc
    )
