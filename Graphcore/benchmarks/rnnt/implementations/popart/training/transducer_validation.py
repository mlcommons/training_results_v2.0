...
def create_model_and_dataflow_for_inference(builder, model_conf, conf, inputs):
    ...
    with builder.virtualGraph(conf.num_pipeline_stages - 1):
        inference_transcription_out = joint_network.joint_transcription_fc(inference_transcription_out,
                                                                           force_recompute=not conf.enable_pipelining)
...
def setup_validation_data_pipeline(conf, transducer_config):
    ...
    return val_loader, val_feat_proc, val_tokenizer, sampler.total_number_of_elements
...
if __name__ == "__main__":
    logger.info("Transformer Transducer validation")

    parser = conf_utils.add_conf_args(run_mode="validation")
    conf = conf_utils.get_conf(parser)
    assert(len(conf.validation_epoch_span) == 2)
    assert(conf.validation_epoch_span[1] >= conf.validation_epoch_span[0])

    runtime_conf = conf_utils.RunTimeConf(conf, run_mode='validation')
    instance_idx = runtime_conf.instance_idx

    transducer_config = config.load(conf.model_conf_file)

    val_loader, val_feat_proc, val_tokenizer, eval_samples = setup_validation_data_pipeline(runtime_conf, transducer_config)
    ...
    device = device_module.acquire_device(conf, runtime_conf.local_num_ipus, runtime_conf)
    inference_session, inference_anchors, inference_inputs, \
        inference_transcription_out, inference_transcription_out_lens = \
    ...
    if conf.wer_out_path and runtime_conf.instance_idx == 0:
        header = ['epoch', 'wer']
        with open(conf.wer_out_path, 'w') as f:
            writer = csv.writer(f)
            writer.writerow(header)

    for epoch in range(conf.validation_epoch_span[0], conf.validation_epoch_span[1] + 1):
        model_dir = os.path.join(conf.model_dir, "checkpoint_" + str(epoch))
        if not os.path.exists(model_dir):
            continue
            
        logger.info("Epoch # {}".format(epoch))
        if mllogger and instance_idx == 0:
            mllogger.start(key=mllog.constants.EVAL_START, metadata={mllog.constants.EPOCH_NUM: epoch})

        decoder_weights_path = os.path.join(model_dir, "decoder_weights_validation.npy")
        update_pytorch_rnnt_model(pytorch_rnnt_model, decoder_weights_path)

        onnx_path = os.path.join(model_dir, "rnnt_checkpoint_validation.onnx")
        ...
        if conf.wer_out_path and runtime_conf.instance_idx == 0:
            with open(conf.wer_out_path, 'a') as f:
                writer = csv.writer(f)
                writer.writerow([epoch, wer])
            
        epoch_wers[epoch] = wer

        if mllogger and instance_idx == 0:
            mllogger.end(key=mllog.constants.EVAL_STOP, metadata={mllog.constants.EPOCH_NUM: epoch})
            mllogger.event(key=mllog.constants.EVAL_ACCURACY, value=wer, metadata={mllog.constants.EPOCH_NUM: epoch})

        if wer < best_wer:
            best_wer = wer
            best_epoch = epoch
            if best_wer <= conf.wer_target:
                break

    if mllogger and instance_idx == 0:
        mllogger.end(key=mllog.constants.RUN_STOP, value={mllog.constants.STATUS: "success"}, metadata={mllog.constants.STATUS: "success"})
            
    logger.info("WERs by epoch:")
    for epoch in epoch_wers.keys():
        logger.info("Epoch: {} WER: {}".format(epoch, epoch_wers[epoch]))
    logger.info("Best WER for epoch: {} = {}".format(best_epoch, best_wer))
