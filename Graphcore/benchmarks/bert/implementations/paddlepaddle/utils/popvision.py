...
def get_profile_logging_handler(append=False):
    ...
    if directory is None:
        raise RuntimeError(
            "Poplar Engine Option 'autoReport.directory' was not set. Please specify using '--profile-dir' or as an environment variable."
        )
    os.makedirs(directory, exist_ok=True)
    mode = 'w'
    if append:
        mode = 'a'
    profile_handler = logging.FileHandler(
        os.path.join(directory, "app_log.txt"), mode)
...
def set_logging_vars():
    ...
    if directory is None:
        logger.warning(
            "Poplar Engine Option 'autoReport.directory' is not set. Skipping log capture."
        )
        return

    loggers = {"poplar": "TRACE", "poplibs": "TRACE", "popart": "TRACE"}
...
def save_app_info(args):
    ...
    if directory is None:
        logger.warning(
            "Poplar Engine Option 'autoReport.directory' is not set. Skipping app.json save."
        )
        return
