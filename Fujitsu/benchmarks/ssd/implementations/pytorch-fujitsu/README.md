...
For the following launch, the downloaded pretrained model `resnext50_32x4d-7cdf4587.pth`
should be placed on `torch-model-cache/hub/checkpoints`


## Training on Fujitsu PRIMERGY GX2570M6 (single node) with docker
Launch configuration and system-specific hyperparameters for the Fujitsu PRIMERGY GX2570M6
single node reference are in the `config_GX2570M6.sh` script.

To launch single node training on Fujitsu PRIMERGY GX2570M6 with docker,
