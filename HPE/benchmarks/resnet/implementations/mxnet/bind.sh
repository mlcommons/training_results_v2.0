...
readonly local_rank="${OMPI_COMM_WORLD_LOCAL_RANK}"
...
echo "HPE : local rank : ${local_rank} with ${cores_per_node} cores per node and ${cores_per_gpu} cores per gpu"
...
case "${mem_mode}" in
	...
	echo "membind node with bad local node ${local_node} for rank ${local_rank}"
        ;;
    *.sh)
	source "${mem_mode}"
	if [ -z "${bind_mem:-}" ]; then
	...
	echo "membind node with good node ${bind_mem[${local_rank}]} for rank ${local_rank} "
	;;
    off|'')
        ;;
    *)
...
# Exec
################################################################################
echo "HPE : using numactl arg : ${numactl_args} for rank ${local_rank}"
