...
for _experiment_index in $(seq 1 "${NEXP}"); do
    (
        ...
        # Print system info
        docker exec -i "${_cont_name}" python -c "
...
mlperf_log_utils.mx_resnet_print_event(key=constants.CACHE_CLEAR, val=True)"
        ...
        docker exec -i "${_config_env[@]}" "${_cont_name}" \
	       mpirun --report-bindings -display-topo -display-map --allow-run-as-root --bind-to none --np ${DGXNGPU} ./run_and_time.sh
