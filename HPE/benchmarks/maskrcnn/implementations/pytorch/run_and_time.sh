...
#        CMD=( './bind.sh' '--mem=./675D_topology.sh' '--ib=single' '--cluster=${cluster}' '--' ${NSYSCMD} 'python' '-u' )
#    else
...
CMD=( './bind.sh' '--cpu=./675D_topology.sh' '--ib=single' '--cluster=champollion' '--' ${NSYSCMD} 'python' '-u' )
