...
from bert import dataset_input
from bert import modeling
from bert import optimization
from bert.hparam import HParams
...
## Required parameters
flags.DEFINE_string(
    "bert_config_file", "gs://mlperf-data-us-central/data/bert_pretrain/bert_config.json",
...
def _log_weight_init(mllogger, tensor_name):
  ...
  Keeps track of tensors in weights_init_dict so they are printed only once.
...
def bert_model_fn(features, is_training, mllogger=None):
  ...
  (masked_lm_loss, masked_lm_example_loss,
   masked_lm_log_probs) = get_masked_lm_output(
       ...
       masked_lm_weights, FLAGS.num_partitions, mllogger)
...
def get_bert_model_fn(features, labels, is_training, mllogger=None):  # pylint: disable=unused-argument
  """The model_fn used by TrainAndEvalRunner."""
  if not is_training:
    return None, bert_model_fn(features, is_training, mllogger)
  elif FLAGS.batch_size_buckets is None or len(FLAGS.batch_size_buckets) == 1:
    grads = bert_model_fn(features, is_training, mllogger)
    ...
    def model_fn(batch_size, bucket_seq_len):
      ...
      return lambda: bert_model_fn(sliced_feature, is_training, mllogger)
...
def get_masked_lm_output(bert_config,
  ...
  with tf.variable_scope("cls/predictions", reuse=tf.AUTO_REUSE):
    ...
    with tf.variable_scope("transform"):
      ...
      _log_weight_init(mllogger, "cls/predictions/transform/dense/kernel")
      _log_weight_init(mllogger, "cls/predictions/transform/dense/bias")
      input_tensor = modeling.layer_norm(input_tensor)
      _log_weight_init(mllogger, "cls/predictions/transform/LayerNorm/gamma")
      _log_weight_init(mllogger, "cls/predictions/transform/LayerNorm/beta")
    ...
    _log_weight_init(mllogger, "cls/predictions/output_bias")
...
def get_next_sentence_output(bert_config, input_tensor, labels, mllogger=None):
  ...
  with tf.variable_scope("cls/seq_relationship", reuse=tf.AUTO_REUSE):
    ...
    _log_weight_init(mllogger, "cls/seq_relationship/output_weights")
    output_bias = tf.get_variable(
        "output_bias", shape=[2], initializer=tf.zeros_initializer())
    _log_weight_init(mllogger, "cls/seq_relationship/output_bias")
...
def run_pretraining(hparams):
  ...
  def eval_init_fn(cur_step):
    ...
    # in all mllog calls epochs are steps, and examples are sequences.
    mllogger.start(
        "block_start",
        metadata={
  ...
  mllogger.event("submission_platform", "tpu-v4-%d" % (FLAGS.num_tpu_cores * 2))
  mllogger.event("submission_status", "cloud")
  mllogger.event("submission_benchmark", "bert")
  ...
  mllogger.event("opt_learning_rate_warmup_steps", FLAGS.num_warmup_steps)
  mllogger.event("num_warmup_steps", FLAGS.num_warmup_steps)
  mllogger.event("start_warmup_step", FLAGS.start_warmup_step)
  mllogger.event("max_sequence_length", FLAGS.max_seq_length)
  mllogger.event("opt_base_learning_rate", FLAGS.learning_rate)
  mllogger.event("opt_lamb_beta_1", FLAGS.beta_1)
  mllogger.event("opt_lamb_beta_2", FLAGS.beta_2)
  mllogger.event("opt_epsilon", 10**FLAGS.log_epsilon)
  mllogger.event("opt_learning_rate_training_steps", FLAGS.num_train_steps)
  mllogger.event("opt_lamb_weight_decay_rate", FLAGS.weight_decay_rate)
  mllogger.event("opt_lamb_learning_rate_decay_poly_power", 1)
  mllogger.event("gradient_accumulation_steps", FLAGS.steps_per_update)
  mllogger.event("max_predictions_per_seq", FLAGS.max_predictions_per_seq)
  seed = int(time.time())
  if FLAGS.repeatable:
    seed = 123
  ...
  # To make the logging consistent with other mlperf models,
  # in all mllog calls, epochs are steps, and examples are sequences.
  mllogger.event("train_samples",
                 FLAGS.num_train_steps * FLAGS.train_batch_size)
  mllogger.event("eval_samples", FLAGS.num_eval_samples)
