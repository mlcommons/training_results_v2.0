...
class TrainAndEvalRunner(object):
  ...
  def __init__(self,
    ...
    options = tf2.config.optimizer.get_experimental_options()
    options.update({"disable_meta_optimizer": True})
    tf2.config.optimizer.set_experimental_options(options)
    ...
    if tpu_cluster_resolver and tpu_cluster_resolver.cluster_spec():
      self.config.cluster_def.CopyFrom(tpu_cluster_resolver.cluster_spec().as_cluster_def())
