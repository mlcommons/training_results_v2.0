...
from mask_rcnn import box_utils
from mask_rcnn import core_assignment_utils
from mask_rcnn import non_max_suppression
...
def log_weight_init(mllogger, tensor_name):
  ...
  Args:
    mllogger: The MLPerf logger class. Used for weight initialization reporting.
...
def rpn_net(features, min_level=2, max_level=6, num_anchors=3, mllogger=None):
  ...
  with tf.variable_scope('rpn_net', reuse=tf.AUTO_REUSE):
    ...
    def shared_rpn_heads(features, num_anchors):
      ...
      features = tf.layers.conv2d(
          ...
          padding='same',
          name='rpn')
      log_weight_init(mllogger, 'RPNHead_conv')
      ...
      scores = tf.layers.conv2d(
          ...
          padding='valid',
          name='rpn-class')
      log_weight_init(mllogger, 'RPNHead_cls')
      ...
      bboxes = tf.layers.conv2d(
          ...
          padding='valid',
          name='rpn-box')
      log_weight_init(mllogger, 'RPNHead_bbox')
...
def faster_rcnn_heads(features,
  ...
  with tf.variable_scope('faster_rcnn_heads'):
    ...
    with tf.device(core_assignment):
      ...
      log_weight_init(mllogger, 'ROI_BOX_FEATURE_EXTRACTOR_fc6')
      net = dense_with_relu(net, units=mlp_head_dim, name='fc7')
      log_weight_init(mllogger, 'ROI_BOX_FEATURE_EXTRACTOR_fc7')
      ...
      class_outputs = tf.layers.dense(
          ...
          bias_initializer=tf.zeros_initializer(),
          name='class-predict')
      log_weight_init(mllogger, 'ROI_BOX_PREDICTOR_cls')
      box_outputs = tf.layers.dense(
          net, num_classes * 4,
          kernel_initializer=tf.random_normal_initializer(stddev=0.001),
          bias_initializer=tf.zeros_initializer(),
          name='box-predict')
      log_weight_init(mllogger, 'ROI_BOX_PREDICTOR_bbox')
  ...
  with tf.variable_scope('mask_rcnn_heads'):
    ...
    with tf.device(core_assignment):
      ...
      for i in range(4):
        ...
        log_weight_init(mllogger, 'ROI_MASK_FEATURE_EXTRACTOR_fcn' + str(i + 1))
      ...
      net = tf.layers.conv2d_transpose(
          ...
          bias_initializer=tf.zeros_initializer(),
          name='conv5-mask')
      log_weight_init(mllogger, 'ROI_MASK_PREDICTOR_fcn5')
      ...
      log_weight_init(mllogger, 'ROI_MASK_PREDICTOR_fcn_logits')
      if num_partitions is not None and num_partitions > 1:
