...
# ==============================================================================
"""MLPerf Logger for training submissions.

Copied from
https://github.com/mlcommons/logging/blob/master/mlperf_logging/mllog/mllog.py
A few changes have been applied to keep the linter happy,
end to enable testing, but the functionality is the same
"""
...
def _encode_log(namespace, time_ms, event_type, key, value, metadata):
  ...
  ordered_key_val_pairs = [('namespace', namespace), ('time_ms', time_ms),
...
class MLLogger(object):
  ...
  def _do_log(self, level, message, clear_line=False):
    ...
    self.events_logged = self.events_logged + 1
  ...
  def _log_helper(self,
    ...
    if metadata:
      if not isinstance(metadata, dict):
        self._do_log(
            logging.WARNING,
            'Metadata is not dictionary, ignored.',
            clear_line=True)
      else:
