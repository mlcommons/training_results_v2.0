...
class EvaluationMetric(object):
  ...
  def __init__(self, filename, use_cpp_extension=True):
  ...
  def evaluate(self):
    ...
    if self.use_cpp_extension:
      ...
      # Add `coco_eval.params.imgIds = list(set(detections[:, 0]))` if the
      # evaluation is on a subset of images. Same applies to mask below.
      coco_eval.evaluate()
      coco_eval.accumulate()
      coco_eval.summarize()
