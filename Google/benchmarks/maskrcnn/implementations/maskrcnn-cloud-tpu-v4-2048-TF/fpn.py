...
def resnet_fpn(features,
  ...
  with tf.variable_scope('resnet_fpn'):
    ...
    # Logging
    for level in range(min_level, _RESNET_MAX_LEVEL + 1):
      mask_rcnn_architecture.log_weight_init(
          mllogger, 'FPN_inner_block' + str(1 + level - min_level))
      mask_rcnn_architecture.log_weight_init(
          mllogger, 'FPN_layer_block' + str(1 + level - min_level))
