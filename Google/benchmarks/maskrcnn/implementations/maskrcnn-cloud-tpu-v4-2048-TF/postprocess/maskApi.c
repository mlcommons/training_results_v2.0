...
void rleInitNoMalloc( RLE *R, siz h, siz w, siz m, uint *cnts ) {
  R->h=h; R->w=w; R->m=m; R->cnts=cnts;
}
...
void rleEncodeSpatialBinary(RLE *R, const byte *M, siz h, siz w ) {
  siz j, k, l; uint c, *cnts=R->cnts; byte p; uint64_t long_p;
  const siz a=w*h;
  static const uint64_t one_mask_long = 0x0101010101010101;
  static const uint64_t zero_mask_long = 0;
  k=0; p=0; long_p = zero_mask_long; c=0;
  uint64_t *long_M = (uint64_t*)M;
  for(j=0; j<a/8; j++) {
    if (long_M[j] == long_p) {
      c += 8;
    } else {
      const siz start=j*8;
      const siz lim=start+8;
      for (l=start; l<lim; l++) {
        if (M[l] != p) { cnts[k++] = c; c = 0; p = M[l]; } c++;
      }
      long_p = p ? one_mask_long : zero_mask_long;
    }
  }
  const siz start=j*8;
  for (l=start; l<a; l++) {
  ...
  cnts[k++]=c; rleInitNoMalloc(R,h,w,k,cnts);
}
