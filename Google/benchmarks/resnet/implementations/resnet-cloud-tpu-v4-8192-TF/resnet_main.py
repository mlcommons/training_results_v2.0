...
from resnet import imagenet_input
from resnet import lars_util
...
def resnet_model_fn(features, labels, is_training, mllogger=None, params=None):
  ...
  Args:
    ...
    mllogger: logger to log weights
  ...
  # Calculate loss, which includes softmax cross entropy and
  # L2 regularization.
...
def get_resnet_model_fn(params):
  """Return wrapper to parametrized resnet_model_fn."""

  def wrapper_fn(features, labels, is_training, mllogger=None):
    return resnet_model_fn(features, labels, is_training, mllogger, params)
...
def run_benchmark(params):
  ...
  def eval_init_fn(cur_step):
    ...
    epoch = (1 + cur_step // FLAGS.steps_per_eval) * 4
    mllogger.start(
        'block_start', metadata={
  ...
  def eval_finish_fn(cur_step, eval_output, summary_writer):
    ...
    params['eval_accuracy'] = eval_accuracy
  ...
  def run_finish_fn(success):
    if not success:
      mllogger.end('run_stop', None, metadata={'status': 'abort'})
  ...
  mllogger.event('submission_platform', 'tpu-v4-%d' % (FLAGS.num_replicas * 2))
  ...
  mllogger.event('lars_opt_weight_decay', FLAGS.weight_decay)
  mllogger.event('lars_opt_momentum', FLAGS.momentum)
  mllogger.event('lars_epsilon', FLAGS.lars_epsilon)
  mllogger.event(
      'model_bn_span',
  ...
  mllogger.event('lars_opt_learning_rate_decay_poly_power', 2.0)
  lars_hp = lars_util.compute_lars_hp(params)
  plr = lars_hp['base_lr']
  w_epochs = lars_hp['w_epochs']
  end_lr = lars_hp['end_lr']
  w_steps = lars_hp['w_steps']
  mllogger.event('lars_opt_base_learning_rate', plr)
  mllogger.event('lars_opt_learning_rate_warmup_epochs', w_epochs)
  mllogger.event('lars_opt_end_learning_rate', end_lr)
  mllogger.event('lars_opt_learning_rate_decay_steps',
  ...
  low_level_runner.initialize(imagenet_train, imagenet_eval,
                              get_resnet_model_fn(params),
                              FLAGS.train_batch_size, FLAGS.eval_batch_size,
                              FLAGS.input_partition_dims,
                              mllogger=mllogger)

  mllogger.event('gradient_accumulation_steps', 1)
  mllogger.event('train_samples', FLAGS.num_train_images)
  mllogger.event('eval_samples', FLAGS.num_eval_images)
...
def main(unused_argv):
  params = {
      ...
      'label_smoothing': FLAGS.label_smoothing,
      'lars_base_learning_rate': FLAGS.lars_base_learning_rate,
      'lars_warmup_epochs': FLAGS.lars_warmup_epochs,
  ...
  for i in range(0, FLAGS.num_experiments):
    ...
    tf.Session.reset(FLAGS.master)  # copybara:strip(internal)
