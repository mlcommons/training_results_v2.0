...
def logits_fn(features, params, feature_config, mllogger=None):
  ...
  for layer_idx in range(len(mlp_dims_bottom)):
    ...
    log_weight_init(mllogger, "bottom_mlp_dense%d" % (layer_idx + 1))
  ...
  if "tc-features" in features:
    ...
    tc_table_sizes = feature_config.get_tc_table_sizes()
    ...
    log_weight_init(mllogger, "embeddings")
  ...
  for layer_idx in range(num_layers_top):
    ...
    log_weight_init(mllogger, "top_mlp_dense%d" % (layer_idx +1))
...
def create_model_fn():
  ...
  def _dlrm_model_fn(features, mode, params, mllogger=None):
    ...
    Args:
      ...
      mllogger: The MLPerf logger class. Used for weight initialization
      reporting.
    ...
    preds, host_call_fn = logits_fn(features, params, None, mllogger)
...
def dlrm_llr_model_fn(params,
  ...
  labels = features["clicked"]
  preds, _ = logits_fn(features, params, feature_config, mllogger)
