...
class FeatureConfig(object):
  ...
  The embedding tables can be placed in REDACTED.
  FeatureConfig sorts the embedding table by its size
  ...
  def __init__(self, params):
    ...
    self._ec_table_indices = list(range(self._num_tables_in_ec))
    if params["selected_tables_on_ec"]:
      self._ec_table_indices = params["selected_tables_on_ec"]
    self._ec_table_indices = sorted(self._ec_table_indices)
    ...
    tf.logging.info(
        "Table vocab sizes in descending order: %s",
        self._table_size_sorted,
    )
    tf.logging.info(
        "Table indices in descending order of size: %s",
        self._table_idx_ordered_by_size,
    )

  def get_tc_table_sizes(self):
    tc_table_sizes = []
    all_table_indices = set(range(self._num_sparse_features))
    tc_table_indices = all_table_indices - set(self._ec_table_indices)
    tc_table_indices = sorted(list(tc_table_indices))
    for tc_idx in tc_table_indices:
      tc_table_sizes.append(self._table_size_sorted[tc_idx])
    return tc_table_sizes
  ...
  def get_feature_tbl_config(self):
    ...
    for i in range(self._num_tables_in_ec):
      tbl_idx = self._ec_table_indices[i]
      vocab_size = self._table_size_sorted[tbl_idx]
    ...
    for i in range(self._num_tables_in_ec):
      feature_idx = offset + i
      feature_to_config_dict.update({
          "categorical-feature-%02d" % feature_idx:
              tpu_embedding.FeatureConfig(table_id="tbl%02d" % (i,))
      })
