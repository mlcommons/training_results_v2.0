...
def create_tpu_estimator_columns(feature_columns, params, iters_per_loop=200):
  ...
  """
  dlrm_tpu_config = tf_estimator.tpu.TPUConfig(
  ...
  return tf_estimator.tpu.TPUEstimator(
      model_fn=dlrm_model.create_model_fn(),
      config=run_config,
      use_tpu=True,
...
def create_tpu_estimator_dicts(feature_to_config_dict,
  ...
  Returns:
    An instance of TPUEstimator to use when training model.
  """
  per_host_train = tf_estimator.tpu.InputPipelineConfig.PER_HOST_V2
  ...
  per_host_eval = tf_estimator.tpu.InputPipelineConfig.PER_HOST_V1
