...
class DLRMEmbeddingRunner(tr.TrainAndEvalRunner):
  ...
  def maybe_add_embedding_enqueue_ops_int(self, is_training, enqueue_ops):
    ...
    for i, batch_data in enumerate(self.enqueue_datas_list[is_training]):
      enqueue_op = tpu_ops.enqueue_tpu_embedding_arbitrary_tensor_batch(
          # Disregard sample_indices_or_row_splits
          sample_indices_or_row_splits=[
              tf.constant(0, shape=(0, 1), dtype=tf.int32)
          ] * len(batch_data),
          embedding_indices=batch_data,
          # Disregard aggregation_weights
          aggregation_weights=[tf.constant(1.0, shape=(0, 1))] *
          len(batch_data),
