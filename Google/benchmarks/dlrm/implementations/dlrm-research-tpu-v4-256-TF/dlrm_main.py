...
from REDACTED.mlperf.submissions.training.v2_0.models.dlrm import common
from REDACTED.mlperf.submissions.training.v2_0.models.dlrm import dataloader
from REDACTED.mlperf.submissions.training.v2_0.models.dlrm import dlrm_embedding_runner
from REDACTED.mlperf.submissions.training.v2_0.models.dlrm import dlrm_model
from REDACTED.mlperf.submissions.training.v2_0.models.dlrm import feature_config as fc
from REDACTED.mlperf.submissions.training.v2_0.models.dlrm import utils
from REDACTED.mlperf.submissions.training.v2_0.models.util import mllog
from REDACTED.mlperf.submissions.training.v2_0.models.util import train_and_eval_runner as tr
from REDACTED.mlperf.submissions.training.v2_0.models.util.roc_metrics.python import roc_metrics
...
def run_model(params,
  ...
  mllogger.event("submission_platform", "tpu-v4-%d" % FLAGS.num_tpu_shards)
  mllogger.event("submission_status", "research")
  mllogger.event("submission_benchmark", "dlrm")
  mllogger.event("submission_division", "closed")
  mllogger.event("submission_org", "google")
  mllogger.event("global_batch_size", params["batch_size"])
  mllogger.event("train_samples", _NUM_TRAIN_EXAMPLES)
  mllogger.event("eval_samples", _NUM_EVAL_EXAMPLES)
  adjusted_lr = params["learning_rate"] * (params["batch_size"] / 2048.0)
  mllogger.event("opt_base_learning_rate", adjusted_lr)
  mllogger.event("sgd_opt_base_learning_rate", adjusted_lr)
  mllogger.event("sgd_opt_learning_rate_decay_poly_power", 2)
  mllogger.event("sgd_opt_learning_rate_decay_steps",
                 params["decay_steps"])
  mllogger.event("lr_decay_start_steps", params["decay_start_step"])
  mllogger.event("opt_learning_rate_warmup_steps",
  ...
  dlrm_model.log_weight_init(mllogger, "embeddings")
  ...
  if embedding:
    runner = dlrm_embedding_runner.DLRMEmbeddingRunner(
        ...
        num_outfeed_threads=min(8, num_hosts),
  ...
  runner.initialize(
      train_input_fn,
      eval_input_fn,
      functools.partial(dlrm_model.dlrm_llr_model_fn, params, feature_config,
                        mllogger=mllogger),
      params["batch_size"],
  ...
  def _compute_metrics_async(in_q, out_q):
    ...
    while True:
      ...
      eval_num = cur_step // FLAGS.steps_between_evals if FLAGS.steps_between_evals else 0
      epoch_num = 1.0 + (1.0 * (eval_num + 1) / num_evals)
      mllogger.start("eval_start", metadata={"epoch_num": epoch_num})
      tf.logging.info(
      ...
      mllogger.end("eval_stop", metadata={"epoch_num": epoch_num})
  ...
  def _async_eval_finish_fn(cur_step, eval_output, summary_writer=None):
    eval_num = cur_step // FLAGS.steps_between_evals if FLAGS.steps_between_evals else 0
    epoch_num = 1.0 + (1.0 * (eval_num + 1) / num_evals)
    mllogger.end("block_stop", metadata={"first_epoch_num": epoch_num})
  ...
  if not success:
    mllogger.end("run_stop", metadata={"status": "failure"})
