...
flags.DEFINE_bool("save_graph", default=False, help=("Save TF graph"))
...
_PROFILE_TIME = flags.DEFINE_integer(
    "profile_time",
    default=2000,
    help=("Number of milliseconds to profile for."))

flags.DEFINE_bool(
    "disable_eval_while_loop",
    default=False,
    help=("Disable eval while loop when there is only one step."))
...
_NUM_CORES_TO_COMPUTATION_SHAPE_MEGACORE = {
    1: [1, 1, 1, 1],
    2: [2, 1, 1, 1],
    4: [2, 2, 1, 1],
    8: [4, 2, 1, 1],
...
class TrainAndEvalRunner(object):
  ...
  def __init__(self,
    ...
    if tpu_cluster_resolver and tpu_cluster_resolver.cluster_spec():
      self.config.cluster_def.CopyFrom(
          tpu_cluster_resolver.cluster_spec().as_cluster_def())
  ...
  def launch_profiler(self):
    ...
    profile_dir = os.path.join(FLAGS.model_dir, 'train')
    tf2.profiler.experimental.client.trace(
      self.master, profile_dir, _PROFILE_TIME.value)
  ...
  def initialize(self,
                 ...
                 params=None,
                 num_partitions=None,
                 computation_shape_override=None,
                 mllogger=None):
    ...
    if computation_shape_override:
      computation_shape = computation_shape_override
    else:
      if topology_info.mesh_shape[-1] == 1:
        computation_shape = _NUM_CORES_TO_COMPUTATION_SHAPE_MEGACORE[
    ...
    for replica_id in range(self.num_replicas):
      ...
      tf.logging.info("replica, device, host_id: %d,%s,%d", replica_id,
                      host_device, host_id)
    tf.logging.info("the number of hosts with replica %d",
                    len(self.host_device_ids))
    ...
    def train_step(*arg):
      ...
      # Temporary patch until all models support mllogger in their
      # model function.
      if mllogger is not None:
        train_model_ret = model_fn(features, labels, True, mllogger)
      else:
    ...
    def train_eval_step():
      ...
      with tf.control_dependencies(train_metric):
        if self.eval_steps == 1 and FLAGS.disable_eval_while_loop:
          return tf.group(self.eval_step(tf.constant(0), train_metric))
    ...
    with self.graph.as_default():
      if self.post_train_loop_callbacks:
        ...
        (self.train_eval_op,) = tpu.shard(
            ...
            outputs_from_all_shards=False,
            device_assignment=self.device_assignment)
      if FLAGS.model_dir and FLAGS.save_graph:
  ...
  def train_and_eval(self,
    ...
    def outfeed_thread_fn(hosts, tid):
      ...
      my_dequeue_ops = [[] for _ in range(len(self.dequeue_ops))]
      for item_idx in range(len(self.dequeue_ops)):
        host_outputs = self.dequeue_ops[item_idx]
        for host_idx in hosts:
          my_dequeue_ops[item_idx].append(host_outputs[host_idx])
      for _ in range(self.num_outfeeds):
        for i, t in enumerate(self.output_sess.run(my_dequeue_ops)):
          self.per_thread_eval_output[tid][i].append(list(t))

    def create_outfeed_threads():

      def _host_range(t_idx):
        """Return list of host targets for each thread."""
        num_hosts = len(self.host_device_ids)
      ...
      outfeed_threads = []
      self.per_thread_eval_output = [[] for _ in range(self.num_outfeed_threads)
                                    ]
      for i in range(self.num_outfeed_threads):
        self.per_thread_eval_output[i] = [
            [] for _ in range(len(self.dequeue_ops))
        ]
      for i in range(self.num_outfeed_threads):
        outfeed_threads.append(
            threading.Thread(
                target=outfeed_thread_fn, args=(_host_range(i), i)))
      return outfeed_threads

    def outfeed_master_thread_fn():
      if self.num_outfeed_threads == 1:
        ...
        for thr in outfeed_threads:
          thr.join()
        for i in range(len(self.dequeue_ops)):
          for t in range(self.num_outfeed_threads):
            for _, elem in enumerate(self.per_thread_eval_output[t][i]):
              self.eval_output[i] += list(elem)
    ...
    outfeed_master_thread = threading.Thread(target=outfeed_master_thread_fn)
    outfeed_master_thread.start()
    ...
    if self.eval_steps > 0:
      ...
      for cur_step in step_range:
        ...
        outfeed_master_thread.join()
        eval_output = self.eval_output
        self.eval_output = [[] for _ in self.dequeue_ops]
        if cur_step < self.train_steps - self.iterations_per_loop:
          outfeed_master_thread = threading.Thread(
              target=outfeed_master_thread_fn)
          outfeed_master_thread.start()
