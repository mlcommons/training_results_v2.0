...
def compute_map(labels_and_predictions,
  ...
  if nms_on_tpu:
    assert isinstance(labels_and_predictions, (list, tuple))
    predictions = np.concatenate(labels_and_predictions, axis=0)
    predictions = predictions.astype(np.float32, copy=False)
    predictions = predictions.reshape((-1, 7))
    # Remove predictions lower than threshold, including padding from NMS
    predictions = predictions[predictions[:, 5] >= ssd_constants.MIN_SCORE]
  else:
  ...
  if use_cpp_extension:
    coco_dt = coco_gt.LoadRes(
        predictions if nms_on_tpu else np.array(predictions, dtype=np.float32))
    ...
    coco_dt = coco_gt.loadRes(
        predictions if nms_on_tpu else np.array(predictions))
  ...
  tf.logging.info('Current AP: {:.5f}'.format(stats[0]))
...
def decode_single(bboxes_in,
  ...
  for i, score in enumerate(np.split(scores_in, scores_in.shape[1], 1)):
    ...
    # skip background
    if i == 0 and ssd_constants.USE_BACKGROUND_CLASS:
      continue
