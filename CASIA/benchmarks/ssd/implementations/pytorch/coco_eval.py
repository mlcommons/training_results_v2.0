...
class NVCocoEvaluator(CocoEvaluator):
    def __init__(self, annotations_file, iou_types, num_threads=1):
        assert isinstance(iou_types, (list, tuple))
        self.coco_gt = self.get_coco_gt(annotations_file=annotations_file)
    ...
    def get_coco_gt(annotations_file):
        return COCO(annotation_file=annotations_file, use_ext=True)
    ...
    def synchronize_between_processes(self):
        for iou_type in self.iou_types:
            gathered_results = utils.all_gather(self._results[iou_type])
            self._results[iou_type] = []
    ...
    def accumulate(self):
        for iou_type in self.iou_types:
            ...
            with redirect_stdout(None):
                coco_dt = coco_gt.loadRes(results, use_ext=True) if results else COCO(use_ext=True)
