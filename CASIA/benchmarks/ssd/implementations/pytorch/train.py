...
def main(args):
    ...
    if args.apex_head_fusion:
        ...
        eval_sz = args.val_sz
        eval_last_iter_bs = int(math.ceil(float(eval_sz) % (world_size * args.eval_batch_size) / world_size))
    ...
    status = ABORTED
    if args.test_only:
        coco_evaluator = evaluate(model, data_loader_test, device=device, epoch=None, args=args, sbridge=sbridge)
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            train_one_epoch(model, optimizer, scaler, data_loader, device, epoch, args,
                            ...
                            static_loss=static_loss, static_prologues_out=static_prologues_out, sbridge=sbridge)
            if args.output_dir:
                checkpoint = {
            ...
            coco_evaluator = evaluate(model, data_loader_test, device=device, epoch=epoch+1, args=args, sbridge=sbridge)
            accuracy = coco_evaluator.get_stats()['bbox'][0]
            if args.target_map and accuracy >= args.target_map:
                status = SUCCESS
                break
