...
from ml_perf.logger import log_event
...
def init_train(rank, tcomm, model_dir):
    ...
    # reader = pywrap_tensorflow.NewCheckpointReader(os.path.join(FLAGS.work_dir, 'model.ckpt-5672'))
    # var_to_shape_map = reader.get_variable_to_shape_map()
    var_to_shape_map = [v.name[0:-2] for v in tf.trainable_variables()]
    print(var_to_shape_map)
    for key in var_to_shape_map:
        log_event(constants.WEIGHTS_INITIALIZATION, metadata={'tensor': key})
    all_var = [v.name for v in tf.all_variables()]
    if 'global_step:0' in all_var:
       log_event(constants.WEIGHTS_INITIALIZATION,metadata={'tensor':'global_step'})
