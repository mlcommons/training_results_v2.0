...
def data_file_format(sequence_length, mask_tokens, max_sequences_per_pack):
    return [
        ...
        mask_tokens + max_sequences_per_pack - 1, max_sequences_per_pack,
...
def data_ranges(sequence_length, mask_tokens, vocab_length,
                max_sequences_per_pack):
    return [
        vocab_length, max_sequences_per_pack + 1, 2, sequence_length,
        vocab_length, max_sequences_per_pack + 1, 2, max_sequences_per_pack + 1
    ]
...
class BinaryDataLoader(object):
    ...
    def __init__(self,
        ...
        if len(self.files) == 0:
            raise RuntimeError(
                f"No files found matching glob patterns: {input_files}")
        ...
        self.file_duplication_index = [
            start_data_at_epoch % duplication_factor
        ] * len(self.files)
    ...
    def samples_in_file(self, filename):
        ...
        if (num_bytes % bytes_per_sample) != 0:
            raise RuntimeError(
                f"Input file: {filename} does not align to the size of a sample. Check the dataset was generated correctly"
            )
    ...
    def __len__(self):
        if self.len is None:
            total_bytes = reduce(lambda a, f: a + self.samples_in_file(f),
                                 self.files, 0)
    ...
    def load_file_index(self, file_index):
        ...
        offset_bytes = count * self.file_duplication_index[
            file_index] * self.dtype().itemsize

        new_data = np.fromfile(
            filename, self.dtype, count=count, offset=offset_bytes)
...
class CachedDataLoader(BinaryDataLoader):
    ...
    def __init__(self,
        ...
        self.load_cache()
        need_extra_batch = not self.drop_remainder and (
            self.data_cache[0].shape[0] % self.batch_size != 0)
        self.len = self.data_cache[0].shape[0] // self.batch_size + int(
            need_extra_batch)
        self.num_padding_samples = int(need_extra_batch) * (
            self.batch_size - self.data_cache[0].shape[0] % self.batch_size)
        ...
        if self.data_index + batch_size > self.data.shape[0]:
            ...
            if self.data_index < self.len * self.batch_size:
                ...
                padding = np.broadcast_to(
                    self.data[0],
                    [self.num_padding_samples, self.data.shape[1]])
    ...
    def load_cache(self):
        ...
        for __ in range(self.epochs_to_cache):
            ...
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                future = executor.map(self.load_file_index,
            ...
            def concat_shuffle(buffer, data, indices, samples_per_file):
                cumulative_samples_per_epoch = np.zeros(
                    (len(samples_per_file)), dtype=nb.int32)
                cumulative_samples_per_epoch[1:] = np.cumsum(
                    samples_per_file)[:-1]
            ...
            samples_per_file = np.array(
                [self.samples_in_file(f) for f in self.files])
            total_samples_per_epoch = int(sum(samples_per_file))
            buffer = np.empty([total_samples_per_epoch, self.sample_size],
                              self.dtype)
            ...
            self.file_index = 0
        logger.info(f"Loading {len(self.files)} files from disk took: {time.time() - start_time:3.1f} seconds.")
...
class GeneratedDataLoader(BinaryDataLoader):
    ...
    def __init__(self, *args, length=1, generated_ranges=None, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        if self.generated_ranges is None:
            raise RuntimeError(
                "keyword argument 'generated_ranges' must not be None")
    ...
    def __next__(self):
        ...
        for size, max_value in zip(self.sample_sizes, self.generated_ranges):
            items.append(
                np.random.randint(0, max_value, [self.batch_size, size]))
        self.data_index += 1
        return items
...
def get_bert_dataset(tensor_shapes,
    ...
    if generated_data:
        ...
        dl = GeneratedDataLoader(
            **data_loader_args,
            length=length,
            generated_ranges=data_ranges(sequence_length, mask_tokens,
                                         vocab_length, max_sequences_per_pack))
    else:
        dl = CachedDataLoader(
            **data_loader_args, epochs_to_cache=epochs_to_cache)
