...
def load_tf_weights_in_bert(model, tf_checkpoint_path, use_fast_mha=False):
    ...
    for name, array in zip(names, arrays):
        ...
        if m_name[-11:] == '_embeddings':
            pointer = getattr(pointer, 'weight')
        elif m_name == 'kernel' or (m_name in mha_params and
                                    'bias' not in m_name):
...
def fused_bias_dropout_add(x, bias, residual, prob, is_training):
    ...
    if is_training:
        ...
        out = torch.nn.functional.dropout(
            x + bias, p=prob, training=is_training)
        out = residual + out
    return out
...
class LinearActivation(torch.nn.Linear):
    ...
    def __init__(self, in_features, out_features, act='gelu', bias=True):
        ...
        if isinstance(act, str) or (sys.version_info[0] == 2 and isinstance(
                act, unicode)):  # For TorchScript
...
class LinearDropoutAdd(torch.nn.Linear):
    ...
    def forward(self, input, residual):
        if self.bias is None or not self.fused_bias_fc:
            ...
            linear_out = fused_dense.fused_dense_function(
                input.view(-1, shape_init[-1]), self.weight, self.bias)
        ...
        if self.bias is None:
            result = fused_dropout_add(
                linear_out, residual, self.p, is_training=self.training)
        else:
            if not self.fused_bias_fc:
                result = fused_bias_dropout_add(
                    linear_out,
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        if not config.fused_bias_mha:
            ...
            self.dense = fused_dense.FusedDense(config.hidden_size,
                                                config.hidden_size)
    ...
    def forward(self, hidden_states, input_tensor):
        if not self.fuse_bias:
            ...
            hidden_states = self.dense(hidden_states.view(-1, shape_init[
                -1])).view(shape_init)
        if not self.fused_dropout_add:
            hidden_states = self.dropout(hidden_states)
            hidden_states = hidden_states + input_tensor
        else:
            hidden_states = fused_dropout_add(hidden_states, input_tensor,
...
class FastBertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        if self.fused_dropout_add:
            attention_output = fused_dropout_add(
                multi_head_attention_output, residual, self.p, self.training)
...
class PadFMHABertAttention(nn.Module):
    ...
    def forward(self, input_tensor, cu_seqlens):
        ...
        self_output = self.self(
            input_flat, cu_seqlens, self.max_s, is_training=self.training)
...
class UnpadFMHABertAttention(nn.Module):
    ...
    def forward(self, input_tensor, cu_seqlens, max_s, batch_size=None):
        self_output = self.self(
            input_tensor, cu_seqlens, max_s, is_training=self.training)
        attention_output = self.output(self_output, input_tensor)
        return attention_output
    ...
    def __init__(self, config):
        ...
        if not config.fused_dropout_add:
            if not self.fuse_bias:
                ...
                self.dense = fused_dense.FusedDense(config.intermediate_size,
                                                    config.hidden_size)
        else:
            self.dense = LinearDropoutAdd(
                config.intermediate_size,
                config.hidden_size,
                bias=True,
                p=config.hidden_dropout_prob,
                fused_bias_fc=self.fuse_bias)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.p = config.hidden_dropout_prob
        self.fused_dropout_add = config.fused_dropout_add
        ...
        if self.fused_dropout_add:
            ...
            if not self.fuse_bias:
                ...
                hidden_states = self.dense(
                    hidden_states.view(-1, shape_init[-1]))
                shape_final = shape_init[:-1] + [list(hidden_states.size())[-1]]
                hidden_states = hidden_states.view(shape_final)
            hidden_states = self.dropout(hidden_states)
    ...
    def forward(self,
        ...
        if self.unpad:
            ...
            attention_indices, attention_mask, seqlen, ntokens, cu_seqlens, actual_seqlens, maxseqlen_in_batch = generate_mask(
                ...
                pad=self.pad,
                fuse_mask=self.fuse_mask,
                unpad_fmha=self.unpad_fmha)
            if self.pad == True and self.enable_stream == False:
                hidden_states = hidden_states.view(
                    batch, maxseqlen, hidden_size).permute(
                        1, 0, 2).contiguous().view(batch * maxseqlen,
                                                   hidden_size).contiguous()
            if self.pad == True and self.enable_stream == True:
                hidden_states = hidden_states.view(batch * maxseqlen,
                                                   hidden_size)
            if self.pad == False:
                hidden_states = UnpadInput.apply(
                    hidden_states.view(batch * maxseqlen,
                                       hidden_size).contiguous(),
                    attention_indices, batch, maxseqlen, hidden_size, ntokens)
                ...
                if seqlen is None and batch is None:
                    ...
                    if self.unpad_fmha:
                        ...
                        hidden_states = layer_module(
                            hidden_states, attention_mask, seqlen, batch)

                if output_all_encoded_layers:
                    if self.fused_mha and not self.pad_fmha:
                        all_encoder_layers.append(
                            hidden_states.permute(1, 0, 2).contiguous())
                    else:
        ...
        if self.unpad:
            if self.pad == True and self.enable_stream == False:
                hidden_states = hidden_states.view(
                    maxseqlen, batch, hidden_size).permute(
                        1, 0, 2).contiguous().view(batch, maxseqlen,
                                                   hidden_size).contiguous()
            if self.pad == True and self.enable_stream == True:
                hidden_states = hidden_states.view(batch, maxseqlen,
                                                   hidden_size)
            if self.pad == False:
                hidden_states = PadInput.apply(
                    hidden_states, attention_indices, batch, maxseqlen,
                    hidden_size, ntokens).view(batch, maxseqlen,
                                               hidden_size).contiguous()
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states, cu_seqlens):
        ...
        # to the first token.
        if self.pad_fmha:
            first_token_tensor = torch.index_select(
                hidden_states.view(-1, self.hidden_size), 0, cu_seqlens[:-1])
        else:
    ...
    def forward(self, sequence_output, pooled_output, masked_lm_labels):
        if self.dense_seq_output:
            ...
            sequence_flattened = torch.index_select(
                ...
                0,
                torch.nonzero(
                    masked_lm_labels.view(-1) != 0, as_tuple=False).squeeze())
    ...
    def from_pretrained(cls,
        ...
        if from_tf:
            ...
            return load_tf_weights_in_bert(
                model, pretrained_checkpoint, use_fast_mha=config.fused_mha)
    ...
    def forward(self,
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            if not self.dense_seq_output:
                ...
                mlm_predictions_scores = prediction_scores_flat
                mlm_predictions = mlm_predictions_scores.argmax(dim=-1)
                mlm_acc = ((mlm_predictions == mlm_labels) * valid_mask).sum(
                    dtype=torch.float) / num_valid
