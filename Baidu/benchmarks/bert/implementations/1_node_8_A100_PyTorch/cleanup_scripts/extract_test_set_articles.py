...
with open(output_filename, mode='w', newline='\n') as ofile:
        ...
        with open(filename, mode='r', newline='\n') as ifile:
            ...
            for item in articles_in_file_tmp:
                if item.rstrip() != '':
                    ...
                    if hashlib.md5(item.rstrip().encode('utf-8')).hexdigest(
                    ) in test_hashes:
                        print("article found at", idx)
                        target_article_idx = idx
                        found_hashes.append(
                            hashlib.md5(item.rstrip().encode('utf-8'))
                            .hexdigest())
    ...
    if len(test_articles) != 500:
        print("Entering missing article debug section.",
              len(found_hashes), "hashes found.")
        missing_articles = []
    ...
    assert len(
        test_articles
    ) == 500, 'Not all articles were found in shards. Incomplete test set.'
