...
def loader(idx):
    ...
    f_next_sentence_labels = h5_ifile['next_sentence_labels'][:].astype(
        np.int64)
...
for i in range(n_output_shards):
    ...
    print("shard index:", i, ", n_samples_in_this_shard =",
          n_samples_in_this_shard)
    ...
    with h5py.File(ofile, 'w') as f:
        ...
        for local_index in range(n_samples_in_this_shard):
            o_input_ids[local_index] = f_retrieve(
                global_index_offset + local_index, f_input_ids)
            o_input_masks[local_index] = f_retrieve(
                global_index_offset + local_index, f_input_masks)
            o_segment_ids[local_index] = f_retrieve(
                global_index_offset + local_index, f_segment_ids)
            o_masked_lm_positions[local_index] = f_retrieve(
                global_index_offset + local_index, f_masked_lm_positions)
            o_masked_lm_ids[local_index] = f_retrieve(
                global_index_offset + local_index, f_masked_lm_ids)
            o_next_sentence_labels[local_index] = f_retrieve(
                global_index_offset + local_index, f_next_sentence_labels)
