...
def generate_mask(attention_mask,
    ...
    if unpad_fmha:
        ...
        indices = torch.nonzero(
            attention_mask.flatten(), as_tuple=False).flatten()
        maxseqlen = seqlen.max().item()
        b = attention_mask.shape[0]
        cu_seqlens = torch.zeros(
            b + 1, device=attention_mask.device, dtype=torch.int32)
    ...
    if pad == False and fuse_mask == True:
        ...
        padded_mask = (padded_mask.unsqueeze(1) * padded_mask.unsqueeze(2)
                       ).unsqueeze(1).half().repeat(1, heads, 1, 1)
        indices_mask = torch.nonzero(
            padded_mask.flatten(), as_tuple=False).flatten()
        mask = torch.zeros(
            [len(indices_mask)], device="cuda", dtype=torch.float16)
...
class PadInput(torch.autograd.Function):
    ...
    def forward(ctx, input, indices, batch, maxseqlen, hidden, ntokens):
        ...
        output = torch.zeros(
            [ntokens, hidden], device="cuda", dtype=torch.float16)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros(
            [ctx.ntokens, ctx.hidden], device="cuda", dtype=torch.float16)
