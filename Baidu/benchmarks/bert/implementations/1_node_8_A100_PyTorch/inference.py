...
def get_predictions(doc_tokens, tokens_for_postprocessing, start_logits,
                    end_logits, n_best_size, max_answer_length, do_lower_case,
    ...
    for start_index in start_indices:
        for end_index in end_indices:
            ...
            if end_index not in tokens_for_postprocessing.token_to_orig_map:
                continue
            if not tokens_for_postprocessing.token_is_max_context.get(
                    start_index, False):
                continue
    ...
    for pred in prelim_predictions:
        ...
        if pred.start_index > 0:  # this is a non-null prediction
            tok_tokens = tokens_for_postprocessing.tokens[pred.start_index:(
                pred.end_index + 1)]
            orig_doc_start = tokens_for_postprocessing.token_to_orig_map[
                pred.start_index]
            orig_doc_end = tokens_for_postprocessing.token_to_orig_map[
                pred.end_index]
    ...
    if can_give_negative_answer:
        ...
        if len(nbest) == 1:
            nbest.insert(
                0, _NbestPrediction(
                    text="", start_logit=0.0, end_logit=0.0))
...
def main():
    ...
    parser.add_argument(
        '--fp16', action='store_true', help="use mixed-precision")
    parser.add_argument(
        "--local_rank", default=-1, help="ordinal of the GPU to use")
    ...
    input_ids = torch.tensor(
        tensors_for_inference.input_ids, dtype=torch.long).unsqueeze(0)
    segment_ids = torch.tensor(
        tensors_for_inference.segment_ids, dtype=torch.long).unsqueeze(0)
    input_mask = torch.tensor(
        tensors_for_inference.input_mask, dtype=torch.long).unsqueeze(0)
    ...
    answer = get_predictions(
        doc_tokens, tokens_for_postprocessing, start_logits, end_logits,
        args.n_best_size, args.max_answer_length, args.do_lower_case,
        args.can_give_negative_answer, args.null_score_diff_threshold)
