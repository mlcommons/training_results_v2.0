    ...
    def forward(ctx,
        ...
        if b < 4:
            ...
            context, S_dmask = mha.fwd(qkv, cu_seqlens, p_dropout, max_s,
                                       is_training, True, zero_tensors, None)
        else:
            context, S_dmask = mha.fwd(qkv, cu_seqlens, p_dropout, max_s,
    ...
    def backward(ctx, dout):
        ...
        if b < 4:
            ...
            dqkv, dp = mha.bwd(dout, qkv, S_dmask, ctx.cu_seqlens,
                               ctx.p_dropout, ctx.max_s, ctx.zero_tensors)
...
class NoopCat(torch.autograd.Function):
    ...
    def forward(ctx, Wq, Wk, Wv, Bq, Bk, Bv, Wqkv, Bqkv, hidden_size):
        ...
        Wtmp = Wqkv.new()
        Wtmp.set_(Wqkv.storage(),
                  Wqkv.storage_offset(), Wqkv.size(), Wqkv.stride())
        Wtmp.requires_grad = True

        Btmp = Bqkv.new()
        Btmp.set_(Bqkv.storage(),
                  Bqkv.storage_offset(), Bqkv.size(), Bqkv.stride())
...
class FMHA(torch.nn.Module):
    def __init__(self, config):
        ...
        def prep_weights(state_dict, prefix, local_metadata, strict,
            ...
            bias = torch.cat([
                bq.view(self.h, self.d), bk.view(self.h, self.d),
    ...
    def forward(self,
        ...
        ctx = FMHAFun.apply(
            qkv.view(-1, 3, self.h, self.d), cu_seqlens, p_dropout, max_s,
            is_training, zero_tensors)
