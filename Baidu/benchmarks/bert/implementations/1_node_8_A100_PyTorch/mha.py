...
class FastUnpadBertSelfAttention(nn.Module):
    def __init__(self,
        ...
        if self.fuse_mask == True and self.fuse_dropout == True:
            ...
            self.softmax = FastMaskSoftmax(
                dim=-1, stream=enable_stream, sync=enable_sync, timer=False)
        else:
            self.softmax = FastSoftmax(
                dim=-1, stream=enable_stream, sync=enable_sync, timer=False)
    ...
    def pytorch_softmax(self, attention_scores, batch, seqlen, heads):
        ...
        attention_probs = torch.zeros(
            ntokens2, device="cuda", dtype=torch.float16)
        ntokens2 = 0
        for i in range(batch):
    ...
    def forward(self,
        ...
        if self.fuse_qkv:
            weight = torch.cat([
                self.query.weight.view(
                    self.num_attention_heads, self.attention_head_size, 1,
                    self.hidden_size), self.key.weight.view(
                        self.num_attention_heads, self.attention_head_size, 1,
                        self.hidden_size), self.value.weight.view(
        ...
        if self.fuse_mask == True and self.fuse_dropout == True:
            attention_probs = self.softmax(
                attention_scores, attention_mask, self.batch, seqlen,
            ...
            if self.apex_softmax == True:
                attention_probs = self.softmax(attention_scores, self.batch,
                                               seqlen, self.num_attention_heads)
            else:
                if self.pad == True:
                    attention_probs = F.softmax(
                        attention_scores.view(batch, self.num_attention_heads,
                                              seqlen[0], seqlen[0]),
                        dim=-1).flatten().contiguous()
                else:
