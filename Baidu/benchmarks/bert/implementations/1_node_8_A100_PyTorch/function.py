...
def graph(func_or_module,
    ...
    with torch.cuda.stream(stream):
        ...
        for _ in range(warmup_iters):
            ...
            args_require_grad = tuple(i for i in functional_args
                                      if i.requires_grad)
            buffer_incoming_grads = tuple(
                torch.empty_like(o) if o.requires_grad else None
                for o in outputs)
            needed_incoming_grads = tuple(b for b in buffer_incoming_grads
                                          if b is not None)
        ...
        buffer_incoming_grads = tuple(
            torch.empty_like(o) if o.requires_grad else None for o in outputs)
        needed_incoming_grads = tuple(b for b in buffer_incoming_grads
                                      if b is not None)
        ...
        buffer_outputs = tuple(o.detach().requires_grad_(o.requires_grad)
                               for o in outputs)
        ...
        if capture_eval:
            ...
            with torch.no_grad():
                ...
                for _ in range(warmup_iters):
                    ...
                    eval_outputs_was_tensor = isinstance(eval_outputs,
                                                         torch.Tensor)
                    eval_outputs = (
                        eval_outputs,
                ...
                eval_outputs = (
                    eval_outputs, ) if eval_outputs_was_tensor else eval_outputs
    ...
    class Graphed(torch.autograd.Function):
        ...
        def forward(ctx, *inputs):
            if func_or_module.training:
                ...
                with torch.no_grad():
                    if capture_eval:
                        for i, arg in zip(sample_args_eval,
...
def main():
    ...
    print(
        "{} graph_before_ddp {} graph_after_ddp {}\n".format(
            args.local_rank, args.graph_before_ddp, args.graph_after_ddp),
        flush=True)
    ...
    model_segment1 = torch.nn.Sequential(
        torch.nn.Linear(D_in, H),
        torch.nn.Dropout(p=0.2),
        torch.nn.Dropout(p=0.4)).cuda()

    model_segment2 = torch.nn.Sequential(
    ...
    if args.graph_before_ddp or pure_eager:
        ...
        model_segment1 = graph(
            model_segment1, (x.clone(), ), (x_eval.clone(), ),
            graph_stream=stream,
            warmup_only=pure_eager)

        model_segment2 = graph(
            model_segment2, (h.clone().requires_grad_(), ),
    ...
    for e in range(2):
        ...
        print(
            "train: {} {} {} {}".format(
                ...
                tuple(p.grad.sum().item()
                      for p in model_segment2.parameters())),
            flush=True)
        ...
        print("eval: {} {}".format(args.local_rank, loss.item()), flush=True)
