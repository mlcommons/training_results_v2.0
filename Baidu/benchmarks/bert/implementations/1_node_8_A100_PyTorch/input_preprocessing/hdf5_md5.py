...
parser.add_argument(
    '--input_hdf5', type=str, required=True, help='Input tfrecord path')
...
for i in range(f['input_ids'].shape[0]):
    row_sums.append(
        h(str(f['input_ids'][i].tolist()).encode('utf-8')).hexdigest())
f.close()
print("{}\t{}".format(
    os.path.basename(args.input_hdf5),
    h(str(row_sums).encode('utf-8')).hexdigest()))
