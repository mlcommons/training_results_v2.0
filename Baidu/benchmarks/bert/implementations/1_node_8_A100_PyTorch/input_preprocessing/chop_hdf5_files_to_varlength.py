...
parser = argparse.ArgumentParser(description="Training data sharding for BERT.")
parser.add_argument(
    '--input_hdf5_dir', type=str, default='hdf5', help='Input hdf5_file path')
parser.add_argument(
    '--output_hdf5_dir', type=str, default='', help='Output hdf5_file path')
parser.add_argument(
    '--num_shards',
    type=int,
    default=2048,
...
input_files = sorted(
    glob.glob(
        input_path + '/part-00???-of-00500.hdf5', recursive=False))
logging.info('n_input_shards = {}'.format(len(input_files)))
logging.info('n_output_shards = {}'.format(n_output_shards))

output_shards_dir = path.join(args.output_hdf5_dir,
                              '{}_shards_varlength'.format(n_output_shards))
try:
    makedirs(output_shards_dir)
except OSError as error:
    logging.info('Output directory : {} already exists. Overwritting ...'.
                 format(output_shards_dir))
...
logging.info('creating {} output file handles.  This could take a while.'.
             format(n_output_shards))
...
ofile_handle = h5py.File('{}{:05d}{}'.format(ofile_prefix, ofile_idx,
                                             ofile_suffix), 'w')
input_ids = ofile_handle.create_dataset(
    'input_ids', (n_samples_in_this_shard, ),
    dtype=h5py.vlen_dtype(np.dtype('int16')),
    compression=hdf5_compression_method)
segment_ids = ofile_handle.create_dataset(
    'segment_ids', (n_samples_in_this_shard, ),
    dtype=h5py.vlen_dtype(np.dtype('int8')),
    compression=hdf5_compression_method)
masked_lm_positions = ofile_handle.create_dataset(
    'masked_lm_positions', (n_samples_in_this_shard, ),
    dtype=h5py.vlen_dtype(np.dtype('int16')),
    compression=hdf5_compression_method)
masked_lm_ids = ofile_handle.create_dataset(
    'masked_lm_ids', (n_samples_in_this_shard, ),
    dtype=h5py.vlen_dtype(np.dtype('int16')),
    compression=hdf5_compression_method)
next_sentence_labels = ofile_handle.create_dataset(
    'next_sentence_labels',
    data=np.zeros(
        n_samples_in_this_shard, dtype="int8"),
    dtype='i1',
    compression=hdf5_compression_method)
...
for ifile in tqdm(input_files, total=len(input_files)):
    ...
    while ifile_entry_idx < f_input_ids.shape[0]:
        if ofile_entry_idx == n_samples_in_this_shard:  # shard is filled up, prepare a next one
            ...
            ofile_handle = h5py.File('{}{:05d}{}'.format(
                ofile_prefix, ofile_idx, ofile_suffix), 'w')
            input_ids = ofile_handle.create_dataset(
                'input_ids', (n_samples_in_this_shard, ),
                dtype=h5py.vlen_dtype(np.dtype('int16')),
                compression=hdf5_compression_method)
        ...
        input_ids[ofile_entry_idx] = f_input_ids[ifile_entry_idx, :sum(
            f_input_masks[ifile_entry_idx])]
        segment_ids[ofile_entry_idx] = f_segment_ids[ifile_entry_idx, :sum(
            f_input_masks[ifile_entry_idx])]
        masked_lm_positions[ofile_entry_idx] = f_masked_lm_positions[
            ifile_entry_idx, :sum(f_masked_lm_positions[ifile_entry_idx] != 0)]
        masked_lm_ids[ofile_entry_idx] = f_masked_lm_ids[ifile_entry_idx, :sum(
            f_masked_lm_positions[ifile_entry_idx] != 0)]
        next_sentence_labels[ofile_entry_idx] = f_next_sentence_labels[
            ifile_entry_idx]
