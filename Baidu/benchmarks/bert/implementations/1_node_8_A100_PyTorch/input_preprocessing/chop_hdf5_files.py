...
output_shards_dir = path.join(
    args.output_hdf5_dir, 'hdf5_{}_shards_uncompressed'.format(n_output_shards))
try:
...
ofile_handles = [
    h5py.File('{}{:05d}{}'.format(ofile_prefix, shard, ofile_suffix), 'w')
    for shard in range(n_output_shards)
]
...
o_masked_lm_positions = np.ndarray(
    (n_samples_in_this_shard, max_predictions_per_seq))
...
for ifile in tqdm(input_files, total=len(input_files)):
    ...
    while ifile_entry_idx < f_input_ids.shape[0]:
        if ofile_entry_idx == n_samples_in_this_shard:
            ...
            o_input_ids = np.ndarray((n_samples_in_this_shard, max_seq_length))
            o_input_masks = np.ndarray(
                (n_samples_in_this_shard, max_seq_length))
            o_segment_ids = np.ndarray(
                (n_samples_in_this_shard, max_seq_length))
            o_masked_lm_positions = np.ndarray(
                (n_samples_in_this_shard, max_predictions_per_seq))
            o_masked_lm_ids = np.ndarray(
                (n_samples_in_this_shard, max_predictions_per_seq))
            o_next_sentence_labels = np.ndarray((n_samples_in_this_shard))
        ...
        o_masked_lm_positions[ofile_entry_idx] = f_masked_lm_positions[
            ifile_entry_idx]
