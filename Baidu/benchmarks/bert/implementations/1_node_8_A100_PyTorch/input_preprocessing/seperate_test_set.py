...
def process_one_file(file_id):
    ...
    logging.info(
        'Processed %s => %s, %d of %d articals picked into test set. %s',
...
if __name__ == '__main__':
    ...
    toc = time.time()
    logging.info('Processed %s (%d files) in %.2f sec', args.data, num_files,
                 toc - tic)
    ...
    with io.open(output_filename, 'w', encoding='utf-8', newline='\n') as fout:
        with io.open(
                hash_filename, 'w', encoding='utf-8', newline='\n') as hashout:
            for f in test_articles_in_files:
                for article in f:
                    fout.write(article)
                    fout.write('\n\n')

                    article_hash = hashlib.md5(article.rstrip().encode(
                        'utf-8')).hexdigest()
