...
class Bmm1Function(torch.autograd.Function):
    ...
    def forward(ctx, batch1, batch2, seqlen, batch, maxseqlen, heads, embed,
        ...
        mhalib.FastBmm1Fprop(batch2.flatten().contiguous(),
    ...
    def backward(ctx, grad_output):
        ...
        mhalib.FastBmm1Dgrad2(batch2.flatten().contiguous(),
                              grad_output.flatten().contiguous(),
                              grad_batch1.flatten().contiguous(), batch, seqlen,
                              heads, embed, ctx.scale, False, ctx.stream,
                              ctx.sync)
        mhalib.FastBmm1Dgrad1(batch1.flatten().contiguous(),
                              grad_output.flatten().contiguous(),
                              grad_batch2.flatten().contiguous(), batch, seqlen,
                              heads, embed, ctx.scale, False, ctx.stream,
                              ctx.sync)
...
class Bmm1(torch.nn.Module):
    ...
    def forward(self, batch1, batch2, batch, seqlen):
        return Bmm1Function.apply(batch1, batch2, seqlen, batch, self.maxseqlen,
...
class Bmm1StridedFunction(torch.autograd.Function):
    ...
    def forward(ctx, mixed, seqlen, batch, maxseqlen, heads, embed, scale,
        ...
        mhalib.FastBmm1Fprop(mixed, mixed, output, batch, seqlen, heads, embed,
                             scale, True, stream, sync)
    ...
    def backward(ctx, grad_output, grad_mixed):
        ...
        mhalib.FastBmm1Dgrad2(mixed, grad_output, grad_mixed, batch, seqlen,
        ...
        mhalib.FastBmm1Dgrad1(mixed, grad_output, grad_mixed, batch, seqlen,
                              heads, embed, ctx.scale, True, ctx.stream,
                              ctx.sync)
