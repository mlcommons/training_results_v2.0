...
class TestEncoders(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.encoder_layers = [
            BertLayer(self.bert_config) for x in range(self.n_encoder_layers)
        ]
        ...
        for layer_idx, encoder_layer in enumerate(self.encoder_layers):
            ...
            pyt_tf_map = unit_test_utils.pyt_tf_mapping(
                state_dict, 'bert/encoder/layer_' + str(layer_idx) + '/')
            for key in pyt_tf_map:
                ...
                if 'dense' in mapped_key or 'query' in mapped_key or 'key' in mapped_key or 'value' in mapped_key:
                    ...
                    state_dict[key] = torch.from_numpy(gv.tf_weights[
                        mapped_key])
            ...
            for key in pyt_tf_map:
                ...
                if 'dense' in mapped_key or 'query' in mapped_key or 'key' in mapped_key or 'value' in mapped_key:
                    new_result = torch.equal(
                        state_dict[key],
                        torch.from_numpy(
                            np.transpose(gv.tf_weights[mapped_key])).to(
                                self.device))
                    test_result = test_result and new_result
                    if not new_result:
                        print("Failure:", mapped_key)
                else:
                    new_result = torch.equal(state_dict[key],
                                             torch.from_numpy(gv.tf_weights[
    ...
    def test_encoder_attention_query(self, layer_idx=0):
        pyt_result = self.encoder_layers[layer_idx].attention.self.query(
    ...
    def test_encoder_attention_key(self, layer_idx=0):
        pyt_result = self.encoder_layers[layer_idx].attention.self.key(
            self.tf_encoder_tensors[layer_idx]['bert/encoder/layer_' + str(
                layer_idx) + '/attention/self/from_tensor'])
        pyt_result = pyt_result.detach().cpu().numpy()
    ...
    def test_encoder_attention_value(self, layer_idx=0):
        pyt_result = self.encoder_layers[layer_idx].attention.self.value(
            self.tf_encoder_tensors[layer_idx]['bert/encoder/layer_' + str(
                layer_idx) + '/attention/self/from_tensor'])
        pyt_result = pyt_result.detach().cpu().numpy()
