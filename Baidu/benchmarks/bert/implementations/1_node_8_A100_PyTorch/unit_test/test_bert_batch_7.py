...
class TestBert(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.bert_config = BertConfig(
            vocab_size_or_config_json_file=self.config_path)
        ...
        if not gv.pyt_model:
            gv.pyt_model = BertForPretraining.from_pretrained(
                self.tf_checkpoint_path, from_tf=True, config=self.bert_config)
        ...
        self.tf_input_mask = torch.from_numpy(
            np.load(self.tf_input_mask_path)).to(self.device)
        ...
        self.tf_input_ids = torch.from_numpy(np.load(
            self.tf_input_ids_path)).to(self.device)

        self.tf_token_type_ids_path = self.data_path + 'input_tensors/segment_ids.np'
        self.tf_token_type_ids = torch.from_numpy(
            np.load(self.tf_token_type_ids_path)).to(self.device)

        self.tf_masked_lm_positions_path = self.data_path + 'input_tensors/masked_lm_positions.np'
        self.tf_masked_lm_positions = torch.from_numpy(
            np.load(self.tf_masked_lm_positions_path)).to(self.device)

        self.tf_masked_lm_ids_path = self.data_path + 'input_tensors/masked_lm_ids.np'
        self.tf_masked_lm_ids = torch.from_numpy(
            np.load(self.tf_masked_lm_ids_path)).to(self.device)

        self.tf_next_sentence_label_path = self.data_path + 'input_tensors/next_sentence_labels.np'
        self.tf_next_sentence_label = torch.from_numpy(
            np.load(self.tf_next_sentence_label_path)).to(self.device)

        self.tf_word_embeddings_output_path = self.data_path + 'intermediate_tensors/bert_embeddings_word_embedding_output.np'
        self.tf_word_embeddings_output = np.load(
            self.tf_word_embeddings_output_path)

        self.tf_position_embeddings_output_path = self.data_path + 'intermediate_tensors/bert_embeddings_position_embeddings.np'
        self.tf_position_embeddings_output = np.load(
            self.tf_position_embeddings_output_path)
        ...
        self.tf_token_type_embeddings_output = np.load(
            self.tf_token_type_embeddings_output_path)
        ...
        self.tf_embedding_summation_output = np.load(
            self.tf_embedding_summation_output_path)
        ...
        self.encoder_key_template = {
            ...
            'bert/encoder/layer_%/attention/self/attention_score_scaled_output':
            None,
            'bert/encoder/layer_%/attention/self/attention_score_additive_output':
            None,
        ...
        for layer_idx in range(self.n_encoder_layers):
            self.tf_encoder_tensors.append({
                key.replace('%', str(layer_idx)):
                torch.from_numpy(gv.tf_tensors[key.replace('%', str(
                    layer_idx))]).to(self.device)
                for (key, value) in self.encoder_key_template.items()
            })
        ...
        for key in pyt_tf_map:
            ...
            if 'dense' in mapped_key or 'query' in mapped_key or 'key' in mapped_key or 'value' in mapped_key:
                state_dict[key] = torch.from_numpy(
                    np.transpose(gv.tf_weights[mapped_key]))
            else:
    ...
    def test_bert_word_embeddings(self):
        pyt_result = self.bert.bert.embeddings.word_embeddings(
            self.tf_input_ids.long())
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, self.tf_word_embeddings_output)
        self.assertTrue(bins_absolute[-1] < self.tolerance)

    def test_bert_position_embeddings(self):
        pyt_result = self.bert.bert.embeddings.position_embeddings(
            self.bert.bert.embeddings.get_position_ids(self.tf_input_ids.long(
            )))
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, self.tf_position_embeddings_output)
        self.assertTrue(bins_absolute[-1] < self.tolerance)

    def test_bert_token_type_embeddings(self):
        pyt_result = self.bert.bert.embeddings.token_type_embeddings(
            self.tf_token_type_ids.long())
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, self.tf_token_type_embeddings_output)
        self.assertTrue(bins_absolute[-1] < self.tolerance)
    ...
    def test_bert_embeddings_summation(self):
        ...
        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, self.tf_embedding_summation_output)
        self.assertTrue(bins_absolute[-1] < self.tolerance)

    def test_bert_embedding(self):
        pyt_result = self.bert.bert.embeddings(self.tf_input_ids.long(),
                                               self.tf_token_type_ids.long())
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, self.tf_embedding_output)
        self.assertTrue(bins_absolute[-1] < self.tolerance)

    @parameterized.expand([(x, ) for x in range(24)])
    def test_bert_encoder_attention_query(self, layer_idx=0):
        pyt_result = self.bert.bert.encoder.layer[
            layer_idx].attention.self.query(self.tf_encoder_tensors[
                layer_idx]['bert/encoder/layer_' + str(layer_idx) +
                           '/attention/self/from_tensor'])
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result,
            self.tf_encoder_tensors[layer_idx]['bert/encoder/layer_' + str(
                layer_idx) + '/attention/self/query_output'].view(
                    -1, 512, 1024).detach().cpu().numpy())
    ...
    def test_bert_encoder_attention_key(self, layer_idx=0):
        pyt_result = self.bert.bert.encoder.layer[layer_idx].attention.self.key(
            self.tf_encoder_tensors[layer_idx]['bert/encoder/layer_' + str(
                layer_idx) + '/attention/self/from_tensor'])
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result,
            self.tf_encoder_tensors[layer_idx]['bert/encoder/layer_' + str(
                layer_idx) + '/attention/self/key_output'].view(
                    -1, 512, 1024).detach().cpu().numpy())
    ...
    def test_bert_encoder_attention_value(self, layer_idx=0):
        pyt_result = self.bert.bert.encoder.layer[
            layer_idx].attention.self.value(self.tf_encoder_tensors[
                layer_idx]['bert/encoder/layer_' + str(layer_idx) +
                           '/attention/self/from_tensor'])
        pyt_result = pyt_result.detach().cpu().numpy()
        ...
        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result,
            self.tf_encoder_tensors[layer_idx]['bert/encoder/layer_' + str(
                layer_idx) + '/attention/self/value_output'].view(
                    -1, 512, 1024).detach().cpu().numpy())
    ...
    def test_bert_encoder_self_attention_output(self, layer_idx=0):
        ...
        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result,
            self.tf_encoder_tensors[layer_idx]['bert/encoder/layer_' + str(
                layer_idx) + '/attention/self/context_layer'].view(
                    -1, 512, 1024).detach().cpu().numpy())
    ...
    def test_bert_encoder_self_attention_score(self, layer_idx=0):
        ...
        q_t = self.bert.bert.encoder.layer[
            layer_idx].attention.self.transpose_for_scores(q)
        k_t = self.bert.bert.encoder.layer[
            layer_idx].attention.self.transpose_key_for_scores(k)
        v_t = self.bert.bert.encoder.layer[
            layer_idx].attention.self.transpose_for_scores(v)

        pyt_result = torch.matmul(q_t, k_t).detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, self.tf_encoder_tensors[layer_idx]
            ['bert/encoder/layer_' + str(layer_idx) +
             '/attention/self/attention_score_output'].detach().cpu().numpy())
    ...
    def test_bert_encoder_self_attention_score_normalized(self, layer_idx=0):
        ...
        pyt_result = pyt_result / math.sqrt(self.bert.bert.encoder.layer[
            layer_idx].attention.self.attention_head_size)
        pyt_result = pyt_result - 10000.0 * (
            1.0 - self.tf_input_mask.float().unsqueeze(1).unsqueeze(2))
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result,
            self.tf_encoder_tensors[layer_idx]['bert/encoder/layer_' + str(
                layer_idx) + '/attention/self/attention_score_additive_output']
            .detach().cpu().numpy())
    ...
    def test_bert_encoder_self_attention_probs(self, layer_idx=0):
        ...
        pyt_result = pyt_result - 10000.0 * (
            1.0 - self.tf_input_mask.float().unsqueeze(1).unsqueeze(2))
        pyt_result = self.bert.bert.encoder.layer[
            layer_idx].attention.self.softmax(pyt_result)
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, self.tf_encoder_tensors[layer_idx]
            ['bert/encoder/layer_' + str(layer_idx) +
             '/attention/self/attention_probs_output'].detach().cpu().numpy())
    ...
    def test_bert_encoder_layers(self, layer_idx=0):
        ...
        batch_size = 7
        model_updated_input_mask = (1.0 - self.tf_input_mask) * -10000.0
        pyt_result = self.bert.bert.encoder.layer[layer_idx](
            self.tf_encoder_tensors[layer_idx]['bert/encoder/layer_' + str(
                layer_idx) + '/attention/self/from_tensor'],
            model_updated_input_mask, seq_len, batch_size)
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result,
            torch.from_numpy(gv.tf_tensors['bert/encoder/layer_' + str(
                layer_idx) + '/output/layer_output']).view(-1, 512,
                                                           1024).numpy())
    ...
    def test_bert_pooler_isolated(self):
        model_updated_input_mask = (1.0 - self.tf_input_mask) * -10000.0
        pyt_result = self.bert.bert.pooler(
            torch.from_numpy(gv.tf_tensors[
                'bert/encoder/layer_23/output/layer_output']).to(self.device),
            model_updated_input_mask)
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, gv.tf_tensors['bert/pooler/pooler_output'])
    ...
    def test_bert_total_loss(self):
        ...
        masked_lm_labels = torch.zeros(
            (batch_size, self.max_seq_len), dtype=torch.long).to(self.device)
        offsets = torch.arange(0, batch_size).to(self.device).view(
            -1, 1) * self.max_seq_len
        masked_lm_positions_offset = masked_lm_positions + offsets
        masked_lm_labels.view(-1)[masked_lm_positions_offset[
            masked_lm_positions_offset != 0]] = masked_lm_ids.view(-1)[
                masked_lm_positions.view(-1) != 0]

        #breakpoint()
        loss, mlm_acc, num_masked = self.bert(
            input_ids=input_ids,
        ...
        tf_result = gv.tf_tensors['cls/predictions/mlm_loss'] + gv.tf_tensors[
            'cls/seq_relationship/nsp_loss']

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, tf_result)
    ...
    def test_bert_mlm_loss(self):
        sequence_output = torch.from_numpy(gv.tf_tensors[
            'cls/predictions/mlm_input_tensor']).to(self.device)
        pooled_output = torch.from_numpy(gv.tf_tensors[
            'bert/pooler/pooler_output']).to(self.device)
        input_ids = self.tf_input_ids.long()
        masked_lm_positions = self.tf_masked_lm_positions.long()
        masked_lm_ids = self.tf_masked_lm_ids.long()
        ...
        prediction_scores, seq_relationship_score = self.bert.cls(
        ...
        if self.bert_config.dense_seq_output:
            masked_lm_labels_flat = masked_lm_labels_flat[masked_lm_labels_flat
                                                          != 0]
    ...
    def test_bert_model_sequence_output(self):
        ...
        sequence_output, pooled_output = self.bert.bert(
            input_ids, segment_ids, input_mask, output_all_encoded_layers=False)

        pyt_result = sequence_output
        pyt_result = pyt_result.detach().cpu().numpy()

        tf_result = gv.tf_tensors[
            'bert/encoder/layer_23/output/layer_output']  # TODO: update this
        ...
        if not test_pass:
            print('bert sequence output max abs diff:', bins_absolute[-1], '@',
                  max_idx)
            print('pyt:', pyt_result)
            print('tf:', tf_result)
    ...
    def test_bert_mlm_compare_tf_tensors(self):
        sequence_output_1 = gv.tf_tensors[
            'bert/encoder/layer_23/output/layer_output']
        sequence_output_2 = gv.tf_tensors['cls/predictions/mlm_input_tensor']

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            sequence_output_1, sequence_output_2)
    ...
    def test_bert_mlm_prediction_scores(self):
        ...
        if not self.bert_config.dense_seq_output:
            ...
            pyt_result = torch.index_select(
                pyt_result,
                0,
                masked_lm_labels.view(-1).nonzero(as_tuple=True)[0])

        pyt_result = pyt_result.detach().cpu().numpy()

        tf_result = gv.tf_tensors['cls/predictions/mlm_logits_bias']
        tf_result = nn.functional.pad(
            torch.from_numpy(tf_result), (0, 6), "constant", 0)
        tf_result = torch.index_select(
            tf_result,
            0,
            masked_lm_positions.view(-1).nonzero(
                as_tuple=True)[0].detach().cpu())
        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, tf_result.detach().cpu().numpy())
    ...
    def test_bert_mlm_transform(self):
        sequence_output = torch.from_numpy(gv.tf_tensors[
            'cls/predictions/mlm_input_tensor_gather']).to(self.device)

        pyt_result = self.bert.cls.predictions.transform.dense(sequence_output)
        pyt_result = self.bert.cls.predictions.transform.transform_act_fn(
            pyt_result)
        pyt_result = pyt_result.detach().cpu().numpy()

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, gv.tf_tensors[
                'cls/predictions/transform/mlm_input_tensor_transformed'])
    ...
    def test_bert_mlm_layernorm(self):
        ...
        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, gv.tf_tensors[
                'cls/predictions/transform/mlm_input_tensor_layernorm'])

        test_pass = bins_absolute[-1] < self.tolerance

        if not test_pass:
            print('bert mlm transform layernorm max abs diff:',
                  bins_absolute[-1])
            print('pyt:', pyt_result)
            print('tf:', gv.tf_tensors[
                'cls/predictions/transform/mlm_input_tensor_layernorm'])
    ...
    def test_bert_mlm_decoder_matmul(self):
        ...
        tf_result = nn.functional.pad(
            torch.from_numpy(gv.tf_tensors[
                'cls/predictions/mlm_logits_matmul']), (0, 6), "constant", 0)

        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, tf_result.numpy())
    ...
    def test_bert_mlm_decoder_bias(self):
        ...
        tf_result = nn.functional.pad(
            torch.from_numpy(gv.tf_tensors['cls/predictions/mlm_logits_bias']),
            (0, 6), "constant", 0)
    ...
    def test_bert_nsp_loss(self):
        ...
        counts, bins_absolute, bins_relative, max_idx = unit_test_utils.max_abs_diff_binning(
            pyt_result, gv.tf_tensors['cls/seq_relationship/nsp_loss'])
    ...
    def SKIP_test_bert_initialized_properly(self):
        ...
        for idx, key in enumerate(pyt_tf_map):
            mapped_key = pyt_tf_map[key]
            if 'dense' in mapped_key or 'query' in mapped_key or 'key' in mapped_key or 'value' in mapped_key:
                new_result = torch.equal(
                    state_dict[key],
                    torch.from_numpy(np.transpose(gv.tf_weights[
                        mapped_key])).to(self.device))
                ...
                new_result = torch.equal(
                    state_dict[key],
                    torch.from_numpy(gv.tf_weights[mapped_key]).to(self.device))
                test_result = test_result and new_result
                if not new_result:
                    print("Failure:", state_dict.keys()[idx], '-->', mapped_key)
