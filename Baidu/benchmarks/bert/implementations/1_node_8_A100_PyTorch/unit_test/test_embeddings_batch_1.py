...
class TestEmbeddings(unittest.TestCase):
    def setUp(self):
        ...
        self.embedding_layer = model.layers.embeddings.BertEmbeddings(
            self.bert_config)
        ...
        with torch.no_grad():
            self.tf_word_embeddings_weights = nn.Parameter(
                torch.from_numpy(gv.tf_weights[self.word_embeddings_key]))
            self.tf_position_embeddings_weights = nn.Parameter(
                torch.from_numpy(gv.tf_weights[self.position_embeddings_key]))
            self.tf_token_type_embeddings_weights = nn.Parameter(
                torch.from_numpy(gv.tf_weights[self.token_type_embeddings_key]))
            self.embedding_layer.LayerNorm.bias = nn.Parameter(
                torch.from_numpy(gv.tf_weights[self.layernorm_beta_key]))
            self.embedding_layer.LayerNorm.weight = nn.Parameter(
                torch.from_numpy(gv.tf_weights[self.layernorm_gamma_key]))
    ...
    def test_word_embeddings(self):
        pyt_result = self.embedding_layer.word_embeddings(
    ...
    def test_position_embeddings(self):
        pyt_result = self.embedding_layer.position_embeddings(
            self.embedding_layer.get_position_ids(self.tf_input_ids.long()))
        pyt_result = pyt_result.detach().cpu().numpy()
    ...
    def test_token_type_embeddings(self):
        pyt_result = self.embedding_layer.token_type_embeddings(
    ...
    def test_embedding(self):
        pyt_result = self.embedding_layer(self.tf_input_ids.long(),
                                          self.tf_token_type_ids.long())
        pyt_result = pyt_result.detach().cpu().numpy()
