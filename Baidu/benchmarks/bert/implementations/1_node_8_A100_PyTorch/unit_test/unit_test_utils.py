...
def max_abs_diff_binning(input_a, input_b):
    ...
    counts, bins_absolute = np.histogram(abs_diff,
                                         np.array(bins_relative) * max_abs_diff)
...
def pyt_tf_mapping(pyt_state_dict, add_prefix=''):
    ...
    converted_strings = [
        add_prefix + x.replace('.', '/').replace('weight', 'kernel')
        for x in pyt_strings
    ]
    ...
    for idx, item in enumerate(converted_strings):
        ...
        if 'decoder' in item:
            item = item.replace('predictions/decoder',
