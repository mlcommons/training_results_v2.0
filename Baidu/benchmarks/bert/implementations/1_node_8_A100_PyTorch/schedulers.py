...
class LinearWarmupPolyDecayScheduler(LRScheduler):
    ...
    def __init__(self,
        ...
        super(LinearWarmupPolyDecayScheduler, self).__init__(optimizer,
                                                             last_epoch)
        ...
        mlperf_logger.log_event(
            ...
            sync=False)
        mlperf_logger.log_event(
            key='opt_lamb_learning_rate_decay_poly_power',
            value=degree,
            sync=False)
        mlperf_logger.log_event(
    ...
    def get_lr(self):
        ...
        cond = (mod_step < self.num_warmup_updates).to(dtype=torch.float32,
                                                       device=mod_step.device)
