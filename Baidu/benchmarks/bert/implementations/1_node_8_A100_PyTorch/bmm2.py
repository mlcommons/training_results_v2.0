...
class Bmm2Function(torch.autograd.Function):
    ...
    def forward(ctx, batch1, batch2, seqlen, batch, maxseqlen, heads, embed,
        ...
        output = torch.empty(
            [ntokens, heads, embed], device="cuda", dtype=torch.float16)
        mhalib.FastBmm2Fprop(batch2.flatten().contiguous(),
    ...
    def backward(ctx, grad_output):
        ...
        grad_batch1 = torch.empty(
            [ntokens2 * heads], device="cuda", dtype=torch.float16)
        grad_batch2 = torch.empty(
            [ntokens, heads * embed], device="cuda", dtype=torch.float16)

        mhalib.FastBmm2Dgrad1(batch2.flatten().contiguous(), grad_output,
                              grad_batch1, batch, seqlen, heads, embed, False,
                              False, ctx.stream, ctx.sync)
        mhalib.FastBmm2Dgrad2(grad_output, batch1, grad_batch2, batch, seqlen,
                              heads, embed, False, False, ctx.stream, ctx.sync)
...
class Bmm2(torch.nn.Module):
    ...
    def forward(self, batch1, batch2, batch, seqlen):
        return Bmm2Function.apply(batch1, batch2, seqlen, batch, self.maxseqlen,
                                  self.heads, self.embed, self.stream,
                                  self.sync)
...
class Bmm2StridedFunction(torch.autograd.Function):
    ...
    def forward(ctx, batch1, mixed, seqlen, batch, maxseqlen, heads, embed,
        ...
        mhalib.FastBmm2Fprop(mixed, batch1, output, batch, seqlen, heads, embed,
                             False, True, stream, sync)
    ...
    def backward(ctx, grad_output):
        ...
        grad_batch1 = torch.empty(
            ntokens2 * heads, device="cuda", dtype=torch.float16)
        grad_mixed = torch.empty(
            [ntokens, heads * 3 * embed], device="cuda", dtype=torch.float16)

        if ctx.timers: ctx.timers['start_dgrad'].record()
        mhalib.FastBmm2Dgrad1(mixed, grad_output, grad_batch1, batch, seqlen,
        ...
        mhalib.FastBmm2Dgrad2(grad_output, batch1, grad_mixed, batch, seqlen,
                              heads, embed, False, True, ctx.stream, ctx.sync)
