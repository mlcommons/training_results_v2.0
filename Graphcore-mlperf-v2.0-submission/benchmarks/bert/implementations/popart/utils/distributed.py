...
def sum_distributed_data(data):
    comm = _get_comm()
    size = comm.Get_size()
    rank = comm.Get_rank()

    if type(data) == int:
        sendbuf = np.array([data], int)
        recvbuf = np.empty([size], int)
    else:
        sendbuf = np.array([data], np.float64)
        recvbuf = np.empty([size], np.float64)
    comm.Allgather(sendbuf, recvbuf)
    data = np.sum(recvbuf, axis=0, keepdims=False)
