...
def rearrange_MLM_to_front_NSP_to_back(packed_input_ids, packed_input_mask, packed_segment_ids, packed_positions,
                                       packed_masked_lm_positions, packed_masked_lm_ids, packed_masked_lm_weights,
                                       packed_next_sentence_positions, packed_next_sentence_labels,
                                       packed_next_sentence_weights):
    # By putting all the MLM tokens together to the front, they can easily be sliced out (rather than requiring a gather)
    # the same goes for the NSP tokens which are placed at the end of the sequence
    max_sequence_length = len(packed_input_ids)


    # Select the MLM tokens
    mlm_selector = packed_masked_lm_positions[packed_masked_lm_weights > 0]

    # Select the NSP tokens
    nsp_selector = [packed_next_sentence_positions[i] for i in range(max(packed_input_mask))]

    # Select the rest of the text
    text_selector = [i for i in range(max_sequence_length) if (i not in mlm_selector) and (i not in nsp_selector)]

    # Sanity check
    assert sum(mlm_selector) + sum(text_selector) + sum(nsp_selector) == max_sequence_length

    # Rearrange text
    to_write = []
    for p in [packed_input_ids, packed_input_mask, packed_segment_ids, packed_positions]:
        to_write.append(np.concatenate((p[mlm_selector], p[text_selector], p[nsp_selector])))

    # MLM as before (but 1 token less)
    to_write.append(packed_masked_lm_ids)
    to_write.append(packed_masked_lm_weights)
    assert np.all(packed_masked_lm_ids[len(mlm_selector):] == 0), ("These masked_lm_ids are invalid:", packed_masked_lm_ids)

    # NSP gets new weights
    num_sequences = packed_input_mask.max()
    packed_nsp_labels = packed_next_sentence_labels[:num_sequences]
    packed_nsp_labels = np.concatenate((np.array((max_sequences_per_pack - num_sequences)*[0], np.int32), packed_nsp_labels))
    packed_nsp_weights = np.array([0]*max_sequences_per_pack, np.int32)
    packed_nsp_weights[-num_sequences:] = 1
    to_write.append(packed_nsp_labels)
    to_write.append(packed_nsp_weights)
...
def create_multi_sequence_example(multi_sequence, max_predictions_per_sequence, max_sequence_length, max_sequences_per_pack):
    ...
    # Rearrange the packed sequence order
    to_write = rearrange_MLM_to_front_NSP_to_back(packed_input_ids, packed_input_mask, packed_segment_ids, packed_positions,
                                                  packed_masked_lm_positions, packed_masked_lm_ids, packed_masked_lm_weights,
                                                  packed_next_sentence_positions, packed_next_sentence_labels,
                                                  packed_next_sentence_weights)
