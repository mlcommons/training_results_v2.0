...
def get_tf_mapping(config, task="PRETRAINING", include_optimizer_state=True):
...
def generate_initializers(config, map_names, load_data, mapping, transform={}):
    ...
    for name, array in zip(map_names, load_data):
        ...
        # config.lamb_accl_dtype is for setting the data type of lamb optimizer state

        lamb_accl_dtype = np.float32
        if config.lamb_accl_dtype == "FLOAT16":
            lamb_accl_dtype = np.float16
        ...
        if popart.reservedAccl1Prefix() in mapping[name]:
            array *= config.loss_scaling
            if array.dtype != lamb_accl_dtype:
                array = array.astype(lamb_accl_dtype)
        elif popart.reservedAccl2Prefix() in mapping[name]:
            array *= (config.loss_scaling * config.loss_scaling)
            if array.dtype != lamb_accl_dtype:
                array = array.astype(lamb_accl_dtype)
