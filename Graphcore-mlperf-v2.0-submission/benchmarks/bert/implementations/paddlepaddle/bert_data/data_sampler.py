...
class DistributedDataSampler(Sampler):
    ...
    def __init__(self,
        ...
        if hasattr(self.data_source, "drop_remainder") and not self.data_source.drop_remainder:
            if self.shuffle != False:
                raise ValueError("Cannot do distributed no-drop remainder with shuffling enabled")
            if len(data_source) % popdist_size != 0:
                raise ValueError("No-drop remainder behavior should be handled by dataset loader")
            if popdist_rank == popdist_size - 1:
                self.num_padding_samples = self.data_source.num_padding_samples
            else:
                self.num_padding_samples = 0     
...
class SampleGenerator:
    def __init__(self, data_source, sampler=None):
        ...
        if hasattr(self.sampler, "num_padding_samples"):
            self.num_padding_samples = self.sampler.num_padding_samples
