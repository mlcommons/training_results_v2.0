...
def create_model_and_dataflow_for_training(builder, model_conf, conf, inputs):
    ...
    transcription_network = transducer_builder.TranscriptionNetwork(builder,
                                                                    ...
                                                                    enable_pipelining=conf.enable_pipelining,
    ...
    # pipelining stages of transcripton net are set inside transducer_builder.TranscriptionNetwork
    ...
    with builder.virtualGraph(conf.num_pipeline_stages-2):
        # prediction-net is placed on 2nd last IPU
    ...
    joint_network_w_rnnt_loss = transducer_builder.JointNetwork_wRNNTLoss(builder,
                                                                          ...
                                                                          custom_op=conf.joint_net_custom_op,
                                                                          enable_pipelining=conf.enable_pipelining)

    with builder.virtualGraph(conf.num_pipeline_stages - 1):
        # joint-net is placed on last IPU
...
def setup_training_data_pipeline(conf, transducer_config):
    ...
    return train_loader, sampler.total_number_of_elements


if __name__ == '__main__':

    logger.info("Transformer Transducer training")
    ...
    mllogger = None
    if conf.mlperf_log_path and instance_idx == 0:
        mllogger = mllog.get_mllogger()
        mlperf_log_parent_dir = os.path.dirname(conf.mlperf_log_path)
        mlperf_log_parent_dirs = []
        while mlperf_log_parent_dir != "" and not os.path.exists(mlperf_log_parent_dir):
            mlperf_log_parent_dirs.append(mlperf_log_parent_dir)
            mlperf_log_parent_dir = os.path.dirname(mlperf_log_parent_dir)
        while len(mlperf_log_parent_dirs) > 0:
            os.mkdir(mlperf_log_parent_dirs[-1])
            mlperf_log_parent_dirs.pop()

        if os.path.isfile(conf.mlperf_log_path):
            os.remove(conf.mlperf_log_path)
        mllog.config(filename=conf.mlperf_log_path)
        mllogger.event(key=mllog.constants.SUBMISSION_BENCHMARK, value="rnnt")
        mllogger.event(key=mllog.constants.SUBMISSION_DIVISION, value="open")
        mllogger.event(key=mllog.constants.SUBMISSION_ORG, value="Graphcore Ltd.")
        mllogger.event(key=mllog.constants.SUBMISSION_STATUS, value="onprem")
        mllogger.event(key=mllog.constants.SUBMISSION_PLATFORM, value="POD" + str(conf.replication_factor * conf.num_pipeline_stages))
        mllogger.event(key=mllog.constants.GLOBAL_BATCH_SIZE, value=conf.batch_size * conf.gradient_accumulation_factor)
        mllogger.event(key=mllog.constants.GRADIENT_ACCUMULATION_STEPS, value=conf.gradient_accumulation_factor)
        mllogger.event(key=mllog.constants.OPT_NAME, value="LAMB")
        mllogger.event(key=mllog.constants.OPT_WEIGHT_DECAY, value=conf.weight_decay)
        mllogger.event(key=mllog.constants.OPT_BASE_LR, value=conf.base_lr)
        mllogger.event(key=mllog.constants.OPT_LAMB_LR_MIN, value=conf.base_lr)
        mllogger.event(key=mllog.constants.OPT_LAMB_BETA_1, value=conf.beta1)
        mllogger.event(key=mllog.constants.OPT_LAMB_BETA_2, value=conf.beta2)
        mllogger.event(key=mllog.constants.OPT_LAMB_LR_DECAY_POLY_POWER, value=conf.lr_exp_gamma)
        mllogger.event(key=mllog.constants.OPT_LAMB_EPSILON, value=1e-9)
        mllogger.event(key=mllog.constants.OPT_LR_WARMUP_EPOCHS, value=conf.warmup_epochs)
        mllogger.event(key=mllog.constants.OPT_LAMB_LR_HOLD_EPOCHS, value=conf.hold_epochs)
    ...
    if instance_idx == 0:
        conf_utils.dump_model_conf(conf)

    train_samples = 0
    if conf.generated_data:
        train_loader = test_transducer.setup_generated_data_pipeline(training_runtime_conf, transducer_config)
    else:
        train_loader, train_samples = setup_training_data_pipeline(training_runtime_conf, transducer_config)

    if mllogger and instance_idx == 0:
    ...
    device = device.acquire_device(conf, training_runtime_conf.local_num_ipus, training_runtime_conf)
    ...
    logger.debug("Loading Custom Joint Net op")
    custom_op_utils.load_custom_joint_net_op()
    ...
    for epoch in range(conf.start_epoch, conf.num_epochs):
        ...
        if mllogger and instance_idx == 0:
            mllogger.start(key=mllog.constants.EPOCH_START, metadata={mllog.constants.EPOCH_NUM: epoch})
        ...
        while True:
            ...
            if step % steps_per_epoch == 0 or conf.one_step:
                break
        ...
        if mllogger and instance_idx == 0:
            mllogger.end(key=mllog.constants.EPOCH_STOP, metadata={mllog.constants.EPOCH_NUM: epoch})
