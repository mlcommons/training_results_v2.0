...
def distribute_mpiranks(rank, size):
    ...
    if FLAGS.num_nodes == 1 and FLAGS.num_eval_games % len(eval_ranks) != 0:
        if len(eval_ranks) > 8:
            eval_ranks = [r for r in list(range(0, 8))]
        elif len(eval_ranks) > 4:
            eval_ranks = [r for r in list(range(0, 4))]
        else:
            eval_ranks = [r for r in list(range(0, 1))]
