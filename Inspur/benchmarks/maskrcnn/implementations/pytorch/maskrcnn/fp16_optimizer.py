...
class FP16_Optimizer(object):
    ...
    def __init__(self,
        ...
        if dynamic_loss_scale:
            ...
            self.cur_scale = torch.tensor(2**16, dtype=torch.float32, device='cuda')
            self.min_scale = torch.tensor(1, dtype=torch.float32, device='cuda')
            self.scale_factor = torch.tensor(2, dtype=torch.float32, device='cuda')
            self.cur_iter = 0
	...
	# for fp16 groups
        ...
        self._update_scale(overflow_buf != 0)
        return
    ...
    def _update_scale(self, skip):
        overflow_scale = self.min_scale.max(self.cur_scale / self.scale_factor)
        new_scale = self.cur_scale
        if (self.cur_iter - self.last_overflow_iter) % self.scale_window == 0:
            new_scale *= self.scale_factor
        self.cur_scale = overflow_scale * skip + new_scale * ~skip

    def _syncing_update_scale(self, skip):
